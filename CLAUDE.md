...
- **Reusable UI (binding — ADR-029):** build once, configure everywhere. Every tabular view uses the **Standard DataTable** from `@hms/ui` (TanStack core, shadcn pattern) with a per-module column/filter configuration — never a module-specific table, toolbar, pagination, column-visibility, filter, action menu, or state component. Row actions use `ActionMenu`, destructive actions `ConfirmDialog`, and blank/failed/pending views the shared `EmptyState` / `ErrorState` / `Skeleton`. Check `@hms/ui` before building any UI; a pattern that appears twice gets extracted.
- **Dates (binding — ADR-030):** every user-facing date is `DD/MM/YYYY` (`DD/MM/YYYY HH:mm` with a time), formatted only through `@hms/utils`. Transport stays ISO-8601. No `toLocaleDateString()`, no per-component formatting, no date library.
