...
| EMR-14 | Vitals configuration is read, not assumed | Hospital with vitals parameters configured | Open a consultation | The Vitals card offers this hospital’s configured parameters — never "This hospital has not configured any vitals to record" when it has | P1 | Functional | doctor | Not run |
| EMR-15 | Save stays reachable | Encounter with several prescriptions and lab orders | Scroll to the bottom of the page | The header stays pinned below the app bar with Save, the sign action, and the saved/unsaved state readable | P1 | Usability | doctor | Not run |
| EMR-16 | Unsaved state is stated | Open encounter | Type into any field, save, then type and undo the typing | Header reads "Unsaved changes" while dirty and "All changes saved" after saving and after the edit is undone by hand | P2 | Functional | doctor | Not run |
| EMR-17 | Reload with unsaved work warns | Open encounter with an unsaved edit | Reload the tab | The browser asks before discarding; cancelling keeps the typed work | P2 | Functional | doctor | Not run |
| EMR-18 | Double-click Save creates one save | Open encounter | Click Save twice as fast as possible | One request, one toast, version advances by one — never two records or two version bumps | P1 | Validation | doctor | Not run |
| EMR-19 | A failed save never signs | Encounter open in two tabs | Save in tab A, then press Sign in tab B and confirm | The stale save is refused with a conflict and the encounter stays a draft — the signature never proceeds over a failed save | P1 | Validation | doctor | Not run |
| EMR-20 | Drug picker is the shared Combobox | Drugs in the master | Type part of a drug name in a prescription row | Searchable list with strength, price and stock; arrow keys move, Enter selects, Esc closes; long names wrap rather than truncate | P1 | Functional | doctor | Not run |
| EMR-21 | Free-text medicine still works | Drug master loaded | Type a medicine that is not in the master | Accepted, with "Not in the drug master; pharmacy will match it by hand"; saves and reaches the pharmacy | P1 | Functional | doctor | Not run |
| EMR-22 | Lab test picker matches the drug picker | Tests in the master | Type part of a test name | The same searchable control and keyboard behaviour; picking one fills the code and shows the price | P1 | Functional | doctor | Not run |
| EMR-23 | ICD-10 picker searches the server | — | Type three letters in the diagnosis field | Results arrive debounced with the code beside the term; "Loading…" while in flight, and a clear empty message when nothing matches | P1 | Functional | doctor | Not run |
| EMR-24 | Prescription row alignment | Encounter with a matched and an unmatched drug on two rows | Compare the rows at desktop, tablet and mobile widths | Every label and input top-aligns on a common baseline; a row carrying a hint does not stretch its neighbours; the page never scrolls sideways | P2 | UI | doctor | Not run |
| EMR-25 | Add actions sit in the card footer | Open encounter | Look at the Prescriptions and Lab orders cards | "Add medicine" and "Add test" are in the card footer at the end of the list, not in the card header | P2 | UI | doctor | Not run |
| EMR-26 | Blank repeatable rows are declared | Open encounter | Add a prescription row and leave the drug empty | The footer says the row will not be saved; after Save the row is gone and nothing else was lost | P2 | Usability | doctor | Not run |
| EMR-27 | Deleting a saved prescription confirms | Encounter with a saved prescription | Remove the row | Shared "Delete prescription?" dialog naming the drug; Cancel keeps it; Delete removes it on the next save | P1 | Functional | doctor | Not run |
| EMR-28 | Deleting an unsaved row does not confirm | Open encounter | Add a row, then remove it | Removed immediately with no dialog — nothing was in the record to destroy | P3 | Usability | doctor | Not run |
| EMR-29 | No browser dialogs anywhere | Open encounter | Sign, and delete a saved prescription | Both use the themed application dialog; nothing shows "localhost:3001 says" or the browser default chrome, in Light and Dark | P1 | UI | doctor | Not run |

### Amending a signed consultation (ADR-134)

| ID | Case | Preconditions | Steps | Expected result | Priority | Type | Role | Status |
|---|---|---|---|---|---|---|---|---|
| AMD-01 | A signed note is read-only | Signed consultation | Open it | Every field is read-only; the header shows Signed with the date, Print prescription, and Amend consultation | P1 | Functional | doctor | Not run |
| AMD-02 | No permission, and the screen says which | Role with `emr.encounter.view` but not `emr.encounter.amend` | Open a signed consultation | No Amend button; a panel names the permission in words and as `emr.encounter.amend`, and says who to ask | P1 | Security | receptionist | Not run |
| AMD-03 | A reason is required | Signed consultation | Press Amend and submit an empty or one-word reason | Refused inline with the reason field marked; the consultation stays signed | P1 | Validation | doctor | Not run |
| AMD-04 | Reopening states what it preserves | Signed consultation | Press Amend and read the dialog | It says the signed note is preserved, that the amendment records the name, time, reason and changed fields, and that nothing is overwritten | P1 | Usability | doctor | Not run |
| AMD-05 | Amending unlocks the note and says so | Signed consultation | Amend with a reason | The note becomes editable; a banner names the reason, its author and the original signing time; the header offers Save, Sign amendment and Discard amendment | P1 | Functional | doctor | Not run |
| AMD-06 | The history keeps the note during the amendment | Encounter open for amendment | Open the patient chart | The consultation is still listed in clinical history — not hidden while it is being corrected | P1 | Integration | doctor | Not run |
| AMD-07 | The pharmacy queue keeps the prescription | Signed encounter with an undispensed prescription, then amended | Open the pharmacy pending list | The prescription is still waiting; a correction upstairs does not take a patient’s medicine off the counter | P1 | Integration | pharmacist | Not run |
| AMD-08 | Only the opener may edit | Amendment opened by the doctor | Try to save as the org admin | Refused with "Another user is amending this consultation"; the administrator can open their own amendment instead | P1 | Security | org_admin | Not run |
| AMD-09 | Two amendments cannot be open at once | Amendment already open | Press Amend again as another permitted user | Refused with a conflict; still exactly one amendment on the record | P2 | Validation | org_admin | Not run |
| AMD-10 | Re-signing records what changed | Amendment open, one field corrected | Save, then Sign amendment and confirm | The record locks; the Amendments card lists it as Recorded with author, time, reason and "Changed: Subjective" — naming only the field that changed | P1 | Functional | doctor | Not run |
| AMD-11 | Changing nothing is a real answer | Amendment open, nothing edited | Sign amendment | Recorded as "Reopened and re-signed without changing anything" — not as an empty or missing entry | P2 | Functional | org_admin | Not run |
| AMD-12 | Discarding is possible only before a save | Amendment open | Discard before saving; repeat after saving a change | Before: returns to signed with the note untouched, the discarded attempt still listed. After: refused, telling the user to sign it instead | P1 | Validation | doctor | Not run |
| AMD-13 | The visit completes once | Signed consultation, then amended and re-signed | Check the visit and the audit log | The visit stays completed with its original time; audit holds one `encounter.sign` plus `encounter.amend_open` and `encounter.amend_sign` | P1 | Integration | org_admin | Not run |
| AMD-14 | The trail is append-only | Encounter with a completed, an unchanged and a discarded amendment | Read the Amendments card | All three listed, newest first, each with its own status, author, time and reason; none removable from the UI | P1 | Functional | doctor | Not run |
| AMD-15 | The original survives | Signed consultation whose plan is then amended | Amend, rewrite the plan, re-sign | The chart shows the corrected plan and the trail says the plan changed; the note as signed is retained and was never what the edit wrote over | P1 | Functional | doctor | Not run |
...
| SEL-11 | No native dropdown survives (ADR-135) | Portal, admin console, patient portal | Work through every form and filter screen, or check the DOM for `select` elements | None remain. Every dropdown is the shared control: it follows Light/Dark and the tenant accent, and on a phone it opens the app’s own panel rather than the OS wheel. The single documented exception is the marketing contact form (SEL-23) | P1 | Regression | any | Not run |
| SEL-12 | Rows per page is the shared control | Any list page | Open the rows-per-page control in the table footer | Themed panel with 10/20/50/100 and **no** search box; it sits at the same height as the page buttons beside it; picking 50 refetches and the "Showing 1–50 of N" line updates | P1 | UI/UX | any | Not run |
| SEL-13 | The permission picker is searchable by meaning, not only by key | `/users/{id}` | Open the override permission list and type "payment" | Options are grouped by module and labelled in plain words ("Collect payments") with the key beneath; the search matches both the words and the key | P1 | Usability | org_admin | Not run |
| SEL-14 | Long options wrap instead of truncating | `/admin/tenants/{id}` → Act as | Open the user list | Each option shows the name, the email beneath it and the roles to the right — never one truncated line; the search matches all three | P2 | UI/UX | super_admin | Not run |
| SEL-15 | The stored value is the code, never the label | `/patients/new` | Choose Gender = Female and Blood group = AB-, register, reopen the record and press Edit | The saved record carries `female` and `AB-`, and reopening maps them back to **Female** and **AB-**. The picker never writes a display label, and never a typographic minus | P1 | Functional | receptionist | Not run |
| SEL-16 | Gender and blood group read the same on every screen | `/patients/new`, the patient chart’s edit form, `/providers`, the public self-registration form | Compare the option lists | Identical options in identical order everywhere. Only the **placeholder** differs by audience — "Prefer not to say" on the patient’s own form, "Not specified" at a hospital counter | P2 | UI/UX | any | Not run |
| SEL-17 | A dropdown inside a dialog is not clipped | `/providers` → Weekly schedule | Open the weekday dropdown on the lowest row | The panel renders above the dialog and flips upward when there is no room below — never cut off by the dialog’s scrolling body | P1 | UI/UX | org_admin | Not run |
| SEL-18 | A registry list that failed differs from one that is empty | HFR facility registration with the registry unreachable | Open a registry-backed field | The message says the registry did not answer and warns against submitting blank — distinct from "the registry returned no options for this list", and from a search matching nothing | P1 | UI/UX | org_admin | Not run |
| SEL-19 | A saved registry code survives a slow list | HFR registration with values already saved | Reopen the form and watch a registry field while its list loads | The saved code shows as the selected option, marked as saved earlier while the list loads — never replaced by the placeholder, which would read as "nothing selected" | P1 | Functional | org_admin | Not run |
| SEL-20 | The ABHA address field can be typed into | ABHA creation reaching the address step | Read the hint, then type an address that is not among the suggestions | The hint says "pick one, or type your own" and the field honours it: suggestions filter as you type, and a typed address is accepted | P1 | Functional | receptionist | Not run |
| SEL-21 | Roster weekday keeps its grid track | `/providers` → Weekly schedule with several windows | Compare the weekday control against the time fields beside it | It sits inside its column at the same height as the inputs; the row does not widen or wrap | P2 | UI/UX | org_admin | Not run |
| SEL-22 | Support impersonation finds a person by email or role | `/admin/tenants/{id}` → Act as | Type an email fragment, then a role name | Both narrow the list; the chosen user’s name, email and roles are all readable before confirming | P2 | Usability | super_admin | Not run |
| SEL-23 | The marketing contact form keeps native validation | `/contact` on the marketing site | Submit without choosing a role | The browser refuses and points at the role field. This form is the documented exception to SEL-11 (ADR-135) | P2 | Validation | anonymous | Not run |
