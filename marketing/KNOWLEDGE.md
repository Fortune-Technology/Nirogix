...
**Staging is never crawlable (ADR-045).** `IS_STAGING` in `lib/seo.ts` reads `NEXT_PUBLIC_ENVIRONMENT`, and `app/robots.ts` returns a bare `Disallow: /` with no sitemap or host when it is set. DNS is GoDaddy with no edge access tier, so the staging hosts resolve publicly and would otherwise duplicate the real site's copy on a second hostname. Nginx also sends `X-Robots-Tag: noindex` there — two independent locks, because a mis-edited server block should not be the only thing between unreleased work and a search index.
