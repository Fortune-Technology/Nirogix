...
Append-only. Each entry records **why** an architecturally significant decision was made — not _what_ the code does (KNOWLEDGE.md) or _when_ it shipped (DONE.md). Never rewrite an existing ADR; supersede it with a new one.
...
**Decision:** Persist branding **per tenant (optionally per branch)** in a `tenant_branding` table (tenant-scoped + RLS; nullable `branch_id` = org-wide default with optional branch override): brand/secondary colours, `logo_file_id`, `favicon_file_id`, typography JSON, `updated_at`/optimistic-lock. Logos/favicons are stored via the existing **`FileStorageService`** (no new storage path). The Portal loads the active branding at **session bootstrap** (`GET /branding/current`) and applies it by setting the `--hms-*` CSS variables — the **same seam** the current localStorage demo uses — so **no component changes** are required. Editing is an Org-Admin Settings surface (`platform.branding.manage`): colour picker (primary/secondary), logo + favicon upload with preview, reset-to-default, live preview. Light/Dark stays a per-user toggle; per-tenant _palette_ customization layers on the token override.
...
**Context:** The System Super Admin needs **platform-wide statistics across every tenant** (total orgs, hospitals, doctors, patients, staff, appointments, active/inactive counts, per-module entitlement usage). But the tenancy invariant (#1) and RLS scope _every_ tenant-data query to a single `app.tenant_id`, and invariant #2 forbids one tenant's data leaking to another. A naive "see all rows" path would break isolation and expose PHI across hospitals.
...
1. **`@hms/ui` remains the single canonical component kit.** Shipped product UI comes from `@hms/ui`; shadcn is a source of _patterns and scaffolding_, pulled in with `shadcn add`, then reviewed and restyled before it ships.
...
**Context:** `@hms/ui`'s `DataTable` (83 lines) rendered columns + rows with loading/empty/error states, and 12 Portal screens use it — but it had no sorting, pagination, search, filtering, column visibility, or row selection, so each screen was on course to grow its own. rules.md has always required one table covering all of it; the owner additionally specified the **shadcn/ui Data Table** as the reference implementation, which is a documented _pattern_ over **TanStack Table** (`@tanstack/react-table`) rather than a component you can `shadcn add`.
...
**Context:** ADR-026 built the notification system in `@hms/ui` following the shadcn Toast _pattern_, because at the time shadcn was not installed and the Dependency Rules forbid a second UI library. Once shadcn was installed as a CLI + reference layer, the obvious question was whether to replace ours with the real registry component. `@shadcn/toast` (base-nova) was inspected before deciding: it is Base UI's toast (`@base-ui/react` + shadcn's `button`), and — contrary to the usual hook-only assumption — it exposes a module-level `createToastManager()`, so it _could_ be driven from our API client outside React. It also brings swipe-to-dismiss, stacked-card animation, and upstream maintenance, and its classes (`bg-popover`, `text-muted-foreground`) already resolve to `--hms-*` after the init remap.
...
- **Platform analytics stay aggregate-only** (ADR-023): counts and metrics, never another hospital's row-level records.
  **Consequence:** The sidebar separation and the permission keys are in place now. **Not built, and deliberately not faked:** the support-session/impersonation flow, and the revenue half of the platform dashboard - there is no subscription, plan, or tenant-billing table anywhere in the schema (the `billing` module invoices _patients_, and paid plans are deferred to the Enterprise track by ADR-020), so MRR, ARR, subscription distribution, storage usage, uptime and support tickets have **no data source**. Those tiles will be built when the data exists rather than rendered with invented numbers; the metrics that do exist (tenants, active/inactive, new registrations, users, doctors, branches, module distribution, failed logins, security events, recent activity) are real today.

## ADR-038 - The marketing site states availability, and imagery must earn its place

**Status:** Accepted (this project).
**Context:** The marketing content was PRD-traceable but written in the present tense for the _whole_ catalogue - 25 modules, FHIR, ABDM, DICOM, WhatsApp, payment gateways, "Runs on E2E Networks", "Most popular" on a pricing package - while what exists is Phase 0 plus the MVP 0/1 clinic core, verified locally and not yet deployed. Traceability alone does not stop a customer reading a roadmap as an inventory, and the complaint we are preventing is "your website says you have this, the product does not".
**Decision:** Two rules, both binding and both in `resources/rules.md`.

- **Claim accuracy.** Every claim traces to the PRD, architecture, development plan, a defined phase, or shipped code - and a capability planned for a later phase is **never written as currently available**. Screenshots, mockups, statistics, and illustrations are claims too. The site therefore carries an availability model rather than prose promises: `marketing/lib/availability.ts` defines `built` / `planned`, every module and integration carries a status, each clinic-core module page splits _what it does today_ from _planned for this module_, and a shared `ReleaseNote` states the product's stage wherever the catalogue appears. Security practices are split the same way into _enforced today_ and _commitment_. No popularity or adoption badge exists while we have no customers to count.
...
- **Clinical roles** - doctor, receptionist, pharmacist and lab technician share **one** `ClinicalDashboard` component parameterised by role, because they differ in _which_ work they own, not in how a dashboard reads.
...
**Status:** Accepted (this project) - **supersedes the deployment half of ADR-037** (System Admin stays a separate application _context_, but is now a separate _application_), and **supersedes `resources/domains.md` §5** rows for "Admin / backoffice" and "Customer-facing patient portal", which had decided the opposite.
...
- **The design system is shared, the business logic is not.** Every app imports `@hms/ui`, `@hms/types` and `@hms/permissions`, so typography, forms, tables, dialogs, toasts, empty/error/loading states, theming and accessibility behave identically. Nothing that decides _who may do what_ is duplicated into a frontend.
...
**Context:** `aiportal` was requested as the fifth frontend. There is **no AI capability anywhere in approved scope**: not in the PRD, not in the architecture, not in any phase. `phases.md` places it under _Postponed / Build-as-Sold_ with a condition attached - "requires a CDSCO classification check before any diagnostic-support feature is built" - and ADR-038 plus the marketing capability reference both forbid claiming AI. Building a portal whose product does not exist is how a shell becomes a promise.
...
Two copies is tolerable. Five is a guarantee of drift - and the half most likely to drift is the security-relevant half: _when_ a session is treated as expired, _whether_ a failure is announced, and _whether_ a message from the server is safe to render. A 5xx that leaks a backend internal into a toast is a defect that would then exist in one app and not another, which is worse than having it in both.
...
- **Per app:** the **domain endpoints**. Each frontend builds its own client and exposes only what its audience may call - which is exactly what keeps clinical calls out of the admin console and platform-administration calls out of the Portal (ADR-051). A shared _client_ must not become a shared _API surface_, or the split it supports is undone from the inside.
...
## ADR-056 - Patient self-registration by QR: the token _is_ the tenant, and a submission is a request
...
**Status:** Accepted (this project) - **supersedes the toast implementation in ADR-032.** ADR-026's rule about _who_ raises a notification is untouched and is the reason this swap cost nothing at the call sites.
...
The owner asked for React Toastify. That is a reasonable request on its own terms - it is the more widely known library, and its placement, timing, stacking, pausing and drag-to-dismiss behaviour are well-tested - but the question worth answering was _what must not change while swapping the engine_.
...
- **De-duplication moved into the library.** The de-dupe key _is_ the toast id, and liveness is read with `toast.isActive`. The previous adapter kept its own `Map` of key to id, which could hold an entry for a toast the user had already dismissed; that class of bug is now unrepresentable.
...
- **Status is never carried by colour alone**: every variant renders a distinct icon _and_ a title in words, and errors and warnings additionally take `role="alert"` where the rest take `role="status"`.
...
The second, quieter problem: staging wants a _deterministic_ dataset so automated E2E assertions stay valid, and production wants **no** dataset at all - only the bootstrap configuration a system needs to exist. One file cannot honestly serve three intentions.
...
**Status:** Accepted (this project). Extends ADR-039, which standardised _how_ row actions look; this decides _which must exist_.
...
- **Payment before consultation is a server rule.** Opening a _new_ encounter and moving a visit to `in_consultation` both refuse (409) while the visit's invoice has an outstanding balance. Re-opening an existing encounter is always allowed.
...
- **Infrastructure-name heuristics stay broad, and are documented as separate from the model.** The seeder's `DATABASE_URL` check and env.ts's `R2_BUCKET` marker still recognise a non-production resource by name fragments (`dev`/`local`/`test`/`staging`/…). They guard against a _mislabelled resource_, not against the environment identifier, so their tolerance is deliberate and commented as such.
...
- **Dev/staging quick-login ("Test credentials") now spans both consoles.** It already existed on the Portal (`:3001`); it is added to the **Admin console** (`:3003`) offering the two Platform Admins. Both fill the _same_ form and post to the _same_ API (never a second auth path) and **fold out of a production build** by construction — the account list is built behind the inline `NEXT_PUBLIC_ENVIRONMENT` gate (ADR-071), so the minifier drops it to `[]` in production.
...
- **Credentials split by who they belong to.** NHA issues **one** client id/secret to the registered _application_ (Nirogix) — server configuration, never per tenant, never visible to a hospital. Each _hospital_ registers its own HFR facility and receives its own facility id, which is tenant data (`abdm_facility_config`), is sent as `X-HIP-ID`, and is what the Scan-and-Share callback resolves the tenant from. Putting a facility id in server config would make every hospital transact as whichever one was configured last.
- **ABDM verifies an identity; it never creates a chart.** Every flow ends at a _prefill_ the operator reviews and submits through the ordinary patient endpoint, or at a _link_ onto a chart that already exists. The manual form is untouched and remains the fallback for an unreachable gateway, a failed OTP, a sandbox rate limit, or a patient with no Aadhaar — a registration desk cannot depend on a third party being up.
...
**Context:** ABDM accepts a health record in two forms — a _simple_ bundle wrapping a PDF or image, or a _structured_ bundle carrying coded data — and the standard advice, repeated in the brief we were given, is to ship simple bundles first because they are quicker. NHA also states that every integrator is expected to produce structured data within a couple of years of compliance. Which of the two is actually cheaper depends entirely on what the product already holds.
**Decision:**

- **Structured bundles from the start.** For this codebase the usual advice is inverted. We generate and store **no PDFs** — printing is a browser-rendered route (ADR-047) and the backend has no PDF library — so a "simple" bundle would mean adding a headless-Chromium pipeline purely to manufacture an attachment. Meanwhile the data is already partly coded: **ICD-10** on every diagnosis, **LOINC** on lab tests where the master knows it, and vitals as discrete columns. Structured is the smaller change _and_ the destination, so the detour is skipped.
...
- **Every discovery is audited, matched or not** — and records only which _kinds_ of identifier matched, never their values. A run of unmatched discoveries against similar demographics is what an attempt to enumerate patients looks like, and that pattern is only visible if the misses are recorded too.
- **Contexts linked this way go through the same `markLinkResult` path** as HIP-initiated linking, so `status` means one thing however a context got there.
  **Consequence:** The patient-driven half of M2 is complete and testable against the recording mock — 15 tests, most of them asserting that the right answer is _nobody_. **One caveat is recorded honestly:** the three **inbound** paths ABDM calls us on are not in the Milestone 1 collection and the M2 documentation shows only the gateway side, so they follow the confirmed Scan-and-Share convention and are marked unverified in `abdm.constants.ts` and `BACKLOG.md`. A wrong inbound path fails silently — the gateway simply never reaches us — which is exactly why it is flagged rather than assumed.
...
- **Consent is re-checked immediately before sending, not when the request arrived.** A patient can revoke in the seconds between, and consent artefacts are _deleted_ on revoke (ADR-087) — so the artefact we hold at the instant of sending is the only one that means anything. The request is acknowledged first and the check happens in the worker, which is precisely why it must be re-run there.
...
- **The sweep is an in-process timer, not a queued job.** A queue is right for work that must happen _once_ across a cluster; this must happen _at all_. Every purge is an idempotent DELETE, so concurrent instances cost a little duplicated work and buy independence from Redis being up — a deletion obligation should not be one dependency away from silently not happening. It sweeps the M2 consent store too, which had no scheduler until now.
...
- **An approved Facility ID is adopted as `abdm_facility_config.hipId`**, which M1–M3 already use and an org_admin types by hand today. Two ideas of "which facility are we" is a bug waiting for a busy afternoon. **But a _different_ configured id is never overwritten** — a hospital may be live on one registered by hand months ago, and swapping it underneath a working integration would break every callback silently. That conflict is logged for a human.
...
- **The duplicate check runs before anything is created, and its answer is believed.** Most Indian clinicians already hold an HPR id. A second one is not a spare row — it is a second national identity for a real person, and unpicking that is somebody's afternoon at a government helpdesk. A dedup call that itself _fails_ does not block enrolment, but it is logged loudly, because proceeding blind is exactly how the duplicate happens.
- **No Aadhaar number is stored, returned, or logged.** It is encrypted with ABDM's certificate, sent, and forgotten; the row keeps only ABDM's `txnId`, which is a reference to a verification _they_ hold. A stolen copy of `abdm_staff_hpr` therefore proves nothing about anybody. The OTP is encrypted on the same principle.
...
- **Matching on import is strict, and ambiguity is refused.** Registration number first — unique and externally meaningful — then an exact full name, and only when it identifies exactly one _active_ person. A row matching two people is reported and skipped. This is the mirror of M3's disclosure rule: an HPR id attached to the wrong clinician puts one real person's national identity on another person's record, and **nothing downstream would ever flag it**. A row a human must look at is a nuisance; that is a defect nobody notices.
...
**Consequence.** Three M2 certification cases move from failing to passing — _revoke a granted
request_, _consent status as revoked can be seen in the HMIS_, _records for revoked consent cannot be
seen in the system_ — on logic that was already written. Twelve tests pin the behaviour, including
...
2. **Facility type is not a free-standing field.** Its contract requires **both** an `ownershipCode`
   ...
   _ownership → systems of medicine → facility type → sub-type_, which is a fact about HFR rather
...
_Append new ADRs below with the next number. Never edit an accepted ADR — supersede it._
...
1. **Search is offered before registration, and it is read-only.** One building must hold one
   ...
   The screen therefore sits _before_ the forty-field form in reading order, and is linked from
...
/abdm/registry/facility/update` exists separately from the save route's `PUT`, and the form
   ...
   the status stays `verified`: the facility _is_ registered; what is in flight is a change to its
...
`updateRegistration`, both documented in OpenAPI, with 7 new tests — every one of them a _refusal_,
...
2. **The applicant type is a parameter, not an assumption.** A case's requirement is not the word in
   its own row: NHA scopes whole _sections_ by applicant type, and a section header reading
   `Government` overrides every "Mandatory" beneath it. `CRT_ABHA_301…309` are the proof — each row
   says Mandatory, and they sit under _"Available Only for trusted entities"_. Private: out of scope.
...
**Decision.** `resolveRequirementText()` searches columns D, B, A, C for a value that _looks like_ a
...
NHA documents Fidelius's `--filepath` flag for one reason: a terminal's _"'This command is too long'
(>8192 characters) limitation in case of long input strings"_. Every payload we encrypt is a
...
out this exact field: _"certain HIUs only accept the public key in the base64-encoded X.509 format,
specifically within the key material sent by the HIP, before decryption."_ Both forms work for a HIU
...
from a spreadsheet is safe, deriving _evidence_ from code is not — a function named
...
`dataPushUrl` is `z.string().url()` — the schema comment reads _"accepted as any HTTPS URL by
design"_, which was true of the design and is the point. Every step passed validation.
...
- **401, not the uniform 202.** Hiding _which hospitals exist_ is worth doing; hiding _that this
  endpoint needs a token_ protects nobody and would make a genuine ABDM misconfiguration look like
  success.
...
Smooth scrolling earns its cost on a marketing page, where scrolling _is_ the experience. It earns
...
fixture concern and touches only _when_ - never a status, an amount or a relationship. The audit log
...
arrays, so adding one is not a migration). What a reading _is_ stays typed columns in exact integer
...
`vitals_mode: consultation_only` and `payment_timing: before_consultation` — _exactly_ today's
...
hospital which does not collect vitals at the desk is refused: the permission says _who_ may record,
the mode says _where in the workflow_ recording happens, and a front end cannot opt itself into
either.
...
a reading that fails to save _after_ a successful check-in is logged, not raised — the patient is in
...
The duplication was not only cosmetic. Two things were genuinely missing before the forms _could_
be the same form:
...
- **Nothing recorded how the patient arrived.** `visits.visit_type` exists but answers a different
  question — _where_ the patient is treated (`opd` today, inpatient later). There was no walk-in /
...
`arrival_type: follow_up`, which records _that_ a visit is a return but cannot record _what it
returns to_. The front desk's most ordinary question — "is this a new problem, or are they coming
...
from _not knowing_ a case already exists, so the remedy is to make the open ones impossible to miss
...
**Two permissions, not three.** `opd.case.view` and `opd.case.manage`. Opening a case _is_ part of
...
in the unified visit workflow (ADR-115) offering _no case_ / _an existing open case_ / _a new case_,
...
**department** and **how the patient arrived** (ADR-115's `arrival_type`). A NULL means _any_, which
...
- **Both numbers are kept.** `visits.calculated_fee_paise` holds what the schedule said; the invoice
  holds what was billed. The gap between them _is_ the override, and losing either half makes it
...
It is part of the explanation for every invoice it priced. What a rule _matches on_ cannot be edited
...
patient is, which doctor, and which department. And the desk confirming _is_ the identity check —
...
wall into an oracle answering _"is this mobile number a patient here, and are they due in today?"_
...
implementation. 20 new API tests, most of them asserting what is _not_ disclosed. 710 backend tests
pass.
...
consultations _where permitted_, existing documents, and somewhere to attach the referral letter the
...
leaves its attachment behind reading `(file removed)`, because _that it was once attached_ is part
of the record.
...
goes and looks for it. A named visit or case must belong to _this_ patient. A document filed against
...
**Only this hospital's own records.** Records held by _other_ hospitals are ADBM territory: they
...
**Still open.** Attaching happens against the _patient_, so at check-in the document is not yet
...
scheduling question. So the permission is split: **`abdm.consent.status.view`** says _whether_ a
...
- **No requesting clinician.** The doctor's name and registration number are on the request for the
  _patient's_ benefit (ADR-092), not the desk's.
...
_sold_ is a separate question, and the marketing status stays IN DEVELOPMENT pending NHA
...
stood. It was idempotent in the weak sense: it would not _crash_ on a second run, because most of
...
**The seeder converges on _present_, never on _the dataset's values_.**
...
seeder always runs when the dataset moves. It runs _after_ `db:migrate`, so a table added this week
...
deploy log says _why_ it stopped. **`--reset` is never passed from CI** — rebuilding the dataset
...
A dash is three different statements wearing the same clothes: _nobody has assigned this yet_,
_this cannot have a value_, and _the screen failed to fetch it_. A reader cannot act on it, because
...
Three tabs in Hospital configuration — _Patient registration_, _Online booking_, _Self check-in_ —
...
- One tab where there were three. The sidebar's _Registration requests_, _Booking requests_ and
  _Arrivals_ items are untouched, because those are **queues of work**, not settings, and belong
...
holds one eye icon; opens Appointments and there is no _New appointment_ button. Not a bug — the
...
the administrator _is_ the person who covers the desk when the receptionist is on leave, and the
...
on the next request, can be time-bound, and is audited. The separation is now _available_ rather
...
- **Nothing about enforcement moved.** Every route still runs `requireModule` → `requireCapability`
  → `requirePermission`; the server re-checks each call; RLS is untouched. This changes _what one
  role holds_, not how anything is enforced.
- Documentation that asserted the old split is corrected in the same change: the marketing
  ...
  `testcases.md` PAT-03 and QR-19/QR-20, which used `org_admin` precisely _because_ it lacked these
...
future release would default to _not_ being in it, and the failure mode is silent: no error, no
...
have told them their _role_ was missing something — sending them to ask their administrator (i.e.
...
A key added tomorrow reaches the administrator by default, and _withholding_ one becomes the
...
the administrator's real reach is the _intersection_ of that list with the modules their hospital
...
entitlement set is still loading it is empty, and an empty set is _not_ read as "this hospital has
...
**4. The panel says all of it.** Required permission in words _and_ as a key (the sentence for the
...
- `@hms/client` now depends on `@hms/permissions`. It already depended on the _idea_; the guard is
...
of _Identity / Contact / Emergency contact / Portal access_ — with age nowhere at all, blood group
as the third row of a card called "Identity", and _Patient portal access_ (a desk task) sharing the
...
_Book appointment_ on Appointments and _Check in_ on the OPD queue sit top-right, in the page
header, level with the page title. _Register patient_ on Patients sat one row lower, inside the
...
`ghost` for navigating away (_All patients_), `secondary` for a side task (_Print / PDF_, _Add
item_), the default variant for the action the page exists for. This is what every multi-action
...
A receptionist opening **Book appointment** was met with a _Not permitted — you do not have
permission to perform this action_ toast, beside a form that then worked perfectly.
...
it is _how this hospital runs_, and every staff-facing screen that follows the workflow has to know
...
defaults), so the _page_ was never broken — but the shared API-feedback layer reported the failure
...
Widening the _route_ to any authenticated session was the alternative, and was rejected: it would
...
- The general lesson is worth stating because it will recur: **a permission named for the screen
  that _edits_ something is the wrong key for the screens that merely _read_ it.** When a
...
filled card become _Unnamed · Not specified · DOB unknown · no phone_, above a blank registration
form.

The cause is one line of reasoning that is wrong: **`completeWithProfile` treated the newest
response as _the_ profile.** A verification is several calls and each answers a different amount —
...
2. **The screen said "user not found"** under the Verify button, on a step that had just
   ...
   failure on the _second_ call was reported with the _first_ call's error message.
...
"ABDM did not say" became `false` — _the mobile does not match_ — and the service's
...
`onVerified(res)` before moving to the mobile step. What remains is confirmation of _a phone
number_; it is not permission to know the rest, and holding a complete profile back behind it made
...
says what is true — _"The details below are verified and filled in. Confirming the mobile number
failed: …"_ — instead of claiming the verification failed. Two calls sharing one catch is how a
...
false and _absent_ distinct. An adapter that flattens "not stated" into `false` is not parsing a
...
- **Neither could module entitlement.** Every module was on, so the screens a hospital _has not
  bought_ — the thing ADR-085's whole gating chain exists for — were never absent.
...
patients are inserted _before_ the two deliberately activity-free charts, because
...
effect of the import — the staging one exited the process with code 2, and the _development_ one
...
- **A bigger dataset does not grow an environment that is already seeded.** The clinical story runs
  ...
  _with_ their histories, and `QAHOSP` gains the twelve new charts and catalogue entries but **not**
...
machine seeded last week — which is exactly what the reporter's own dashboard showed: _In the queue
now 0 · Seen today 0_, above six weeks of history.
...
`seedTodayQueue` is extracted from the story and called on every run. Its only question is _does
this hospital already have a visit dated today?_ — if it does, nothing happens; if it does not,
...
- A hospital's history still cannot be lengthened without `--reset` (ADR-132). That remains true
  and is a different problem; what is fixed here is that the _present_ no longer rots.
...
- Two stale references fixed while here: the Arrivals empty state and the check-in poster both
  pointed at _Hospital configuration → Self check-in_, a tab that stopped existing when the three
  QR surfaces were consolidated (ADR-124).
- Setting `vitalsMode: 'after_checkin'` changes what the seeded hospitals _do_, not what the
...
- **The vitals configuration was never fetched.** `workflow` was declared, read in three places,
  and never assigned — so every hospital saw _This hospital has not configured any vitals to
  record_, whatever it had configured. The doctor holds `platform.workflow.view` precisely so this
...
takes a **required reason** and, _before anything becomes editable_, copies the whole signed note
...
whether it _has been_ signed. `encounter.signed` is published once, by the first signature —
...
**7. `Combobox` joins the kit, beside `Select`.** `Select` answers _choose one of these_;
`Combobox` answers _choose one of these, or write your own_, which is the shape every clinical
...
- `Card` gains a `footer`. A repeatable form's _Add another_ belongs at the end of the list it
...
ADR-112 made `Select` the one dropdown, and its own docstring says _reach for it before writing
another `<select className="hms-input">`_. Thirty-seven of those were still in the tree. ADR-134
...
raw lower-case column values (_active_ / _archived_) shown to a person.
...
- Every dropdown in every application now shows a second line where there is one, searches past
  ...
  permission picker is grouped by module and searchable on the key _and_ on what the key means.
...
- Verified by watching the request rather than by reading the code: registering a patient sent
  ...
  record mapped them back to _AB-_ and _Female_.
...
## ADR-136 - A list puts the work at the top, and the sort control does something

**Status:** Accepted · **Date:** 03/09/2026 · **Extends:** ADR-029 (the Standard DataTable), ADR-063 (server-side filtering)

### Context

Default ordering was written per query, and mostly as whatever was easiest at the time. Four of
them were wrong in a way that shows up every day:

- **Appointments** ordered `scheduled_at DESC`, which puts the _furthest-away_ booking at the top.
  A visit three months out sat above this morning's clinic, and today's list was somewhere in the
  middle of page one. Nobody opens a schedule to find out what happens in March.
- **Treatment cases** ordered on the `status` column, ascending. `closed` sorts before `open`, so
  finished cases led the list — and the line above it said, in a comment, _"Open before closed"_.
  The code had contradicted its own comment since it was written.
- **Invoices** ordered `created_at DESC`. A billing list exists to be worked through; a settled
  invoice is a receipt, read on purpose and by number.
- **The laboratory worklist** was flat FIFO across every status, so last week's verified reports —
  which are the bulk of the table — sat on top of this morning's new orders.

Separately, and worse in kind: **the sort control on server-mode tables did nothing.** The
DataTable offers sorting on any column that knows its value and, in server mode, hands the page a
`sort` array. Appointments and invoices forwarded it into `server.sort` and then dropped it from
the API call. The header arrow moved, the URL changed, the rows did not. A control that does
nothing is worse than one that is not offered, because the user concludes the data is already in
that order.

### Decision

**1. Default ordering is chosen per workflow, and written down.** Not one rule; four different
ones, each with the reason in the query:

- **Appointments** — future before past, then _soonest first_ within the future and _most recent
  first_ within the past. Two keys, so each half runs in its own direction.
- **Cases** — ordered on the _meaning_ of the status (`open` = 0), never on the string.
- **Invoices** — outstanding balance first (`total − paid > 0`, and not `void`), then newest.
- **Lab worklist** — by stage (ordered → collected → resulted → verified → cancelled), then FIFO
  _within_ a stage. A status filter makes the first key constant, so a filtered worklist is still
  plain FIFO, which is what a bench wants.

Deliberately **unchanged**: the OPD queue (token order), the pharmacy's pending prescriptions
(FIFO — the queue is already filtered to `ordered`, and the person waiting longest is next), and
the patient directory (newest registered first; it is a directory, not a queue).

**2. A client names a sort key, never a column.** `db/sort.ts` parses the DataTable's own
`key:dir,key:dir` URL format and resolves each key against an **allowlist the module publishes**,
keyed by the same column keys the Portal uses. An unknown key is dropped rather than rejected — a
stale bookmark should give somebody the default list, not a 422 — and dropping every key falls
back to the workflow-aware default. The client string is only ever a map key; it never reaches
SQL, and there is a test that says so with an injection payload.

**3. The default is a default, not a lock.** User sorting now works on the tables that offered it,
which is what ADR-029 promised and what §11.7 asks for.

### Consequences

- A doctor opening Appointments sees tomorrow's clinic first. A cashier opening Billing sees the
  money still owed. A technician opening the worklist sees new orders above finished reports.
- The `case.service` comment and its code agree for the first time.
- Four tests seed sets whose correct order differs from what any single column would give, so a
  reversion to `created_at DESC` fails rather than passing by luck. **All three were confirmed to
  fail against the previous orderings** before the fix was restored.
- Sort is wired through on appointments and invoices only, because they are the server-mode
  tables that offered the control. Other endpoints keep their default; adding a sort key to one
  is now a two-line change against the shared helper.

## ADR-137 - An electronic signature, versioned, and pinned to what it signed

**Status:** Accepted · **Date:** 03/09/2026 · **Extends:** ADR-007 (file storage), ADR-047 (printing), ADR-134 (signed records)

### Context

Every printable document in the product ends in a blank line and a name typed underneath it, and
a hospital prints it so somebody can sign it by hand. The request is to stop doing that.

The thing that makes this dangerous is not the upload. It is what happens **the second time**
somebody uploads. If a prescription renders "the prescribing doctor's signature", then a doctor
who changes their signature next year silently changes what every prescription they have ever
signed shows — including ones already printed, already dispensed, and already in a file. That is
not a display bug; it is a record that says something different from what was signed.

### Decision

**1. Say what it is, everywhere.** An **electronic signature**: an image the user uploaded,
rendered onto a generated document. Not cryptographic, not tamper-evident, no certificate
authority. Stated in the schema comment, the migration, the service docstring, the API
description, the response body (`kind: 'electronic_image'`), and on the card the user sees. It is
the claim most likely to drift in a sales conversation, so it is written where a developer, an
integrator and a user each meet it.

**2. Versions, never edits.** Uploading again inserts a new row and retires the previous one; a
row's `file_id` is never repointed, and nothing is deleted. `removed` means "stop signing new
documents with this", which is the only meaning that can be honoured.

**3. A document pins the version that signed it.** `encounters.signature_id` at signing,
`lab_results.signature_id` at verification. Documents resolve **by that id**, never by "who signed
this, and what do they use now" — which is the question this ADR exists to stop being asked. A
missing pin renders a blank line, exactly as before the feature existed.

**4. Only your own, by construction.** The routes are `/me/signature` and nothing else. There is
no `/users/{id}/signature`, so an administrator holding every permission in the system cannot
upload a signature in a clinician's name — not because a check refuses them, but because the
operation cannot be expressed. `platform.signature.manage` is granted to the roles that sign
something (doctor, lab technician) and reaches `org_admin` by derivation, where it still only
governs their own.

### Consequences

- A prescription prints the doctor's signature above the line, with the signing date under the
  name. A lab report will print the verifier's the same way once its document is built.
- **The rule is tested directly**: sign with version 1, upload version 2, and the document that
  pinned version 1 still resolves version 1 — including after the signature is _removed_.
- Uploading is limited to PNG/JPEG/WebP at 512 KB, through the file module's existing sniffing.
  A signature scanned without transparency prints correctly because the print CSS multiplies it.
- Re-signing an amendment (ADR-134) **re-pins** to the corrector's current signature: the
  corrected note is signed now, by them. The previous state lives in the amendment snapshot.
- Not built: signatures on invoices, medical certificates or discharge documents. Each needs its
  document first; the mechanism is ready and adding one is a `signature_id` column and a prop.

## ADR-138 - One bulk import, and a line under what may be imported

**Status:** Accepted · **Date:** 03/09/2026 · **Extends:** ADR-029 (build once), ADR-060 (row actions), ADR-128 (page actions)

### Context

A hospital arrives with its formulary, its test menu, its price list, its doctors and its patient
register in spreadsheets. Typing them in one at a time is the first thing it is asked to do, and
it is the reason onboarding stalls.

The obvious failure is building a separate importer per module: six file pickers, six sets of
validation, six ideas about what a duplicate is. The less obvious one — and the more serious — is
importing the wrong things.

### Decision

**1. One engine; a module contributes a description of its own data.** The registry entry names
the fields, which are required, what other systems call them (aliases), a realistic example, how
to parse the cell, and — the important one — **which field identifies the same record twice**.
Everything a person experiences is the engine: the template, the automatic mapping, the
validation, the preview, the duplicate strategy, the commit, the run record. Nothing in a module
knows a CSV exists.

**2. Master data, never events.** Importable: medicines, laboratory tests, services, doctors,
departments, patients. **Not importable, and this is the line the brief asked for:** appointments,
visits, consultations, prescriptions, lab results, invoices, payments. Those are produced by a
workflow that checks a patient in, prices a fee, gates on payment and signs a note. A spreadsheet
that creates them skips every one of those checks, and the damage — a patient billed for a
consultation that never happened — is invisible until somebody acts on it. There is a test that
asserts those keys are absent.

**3. A duplicate is a code, never a name.** Medicine code, test code, service code, registration
number, phone. Two doctors can be called Sharma. Asserted for every module by a test.

**4. Preview before commit, and it describes the file rather than reserving anything.** Both calls
take the file; there is no server-side staging area, no temp lifecycle, and no window in which a
hospital's patient register sits half-imported on a disk. The consequence is stated on the screen:
the check is re-run at commit, so a record created by somebody else in between is seen.

**5. Row by row, not one transaction.** Eight hundred medicines where row 412 has a bad price
imports 799 and reports 412. Rolling back would mean re-uploading everything to fix one cell. The
run record is what makes that safe. The exception is `create_only`, chosen _because_ the operator
wants all-or-nothing, which refuses before writing anything.

**6. Patients are create-only.** No `update`. A chart is corrected on the chart by somebody who
can see the duplicate candidates and the audit trail — not silently, in bulk, from a spreadsheet
whose phone column might be a typo. The screen does not offer the option rather than offering it
and refusing.

**7. The module's own permission.** Importing medicines needs the same key as adding one by hand,
because it is the same act at a different scale. Checked inside the handler rather than in the
route table — the only router in the product that does this — because the module is a path
parameter and each carries a different key. `GET /imports` additionally _filters_ to what the
caller may do, so nobody is offered an import that will refuse them (ADR-126).

**8. `import_runs` records the result, not the file.** Who, what module, which file, and the four
counts. The uploaded file is **not** stored: it is the hospital's own data, may carry patient
identifiers, and keeping every upload forever is a liability rather than a feature.

### Consequences

- Six screens gained an **Import** action, in the `PageHeader` before the page's primary action
  (ADR-128), gated by the same permission as creating one record.
- The CSV reader is written here rather than pulled in: quoting, escaped quotes, embedded commas
  and newlines, a BOM and three line endings is about eighty lines, and a parser is a dependency
  that reads every byte a hospital uploads. Excel's BOM is stripped on read and written on
  output, because a template that mangles a ₹ sign is a template nobody trusts.
- Verified against the running Portal with a file using another system's headers
  (`Drug Name` / `SKU` / `MRP`): auto-mapped, one bad price and one in-file duplicate reported
  with the row numbers Excel shows, two rows imported, and a re-import with **update** raising the
  price without creating a second row.
- A 5000-row cap per file, because the preview holds every row and nobody reads 50,000 of them.

## ADR-139 - The half of M1 the matrix could not see, and the API family behind it

**Status:** Accepted · **Date:** 03/09/2026 · **Extends:** ADR-084 (ABDM M1), ADR-100 (certification matrix), ADR-130 (multi-step external flows)

### Context

`abdm-audit.ts` exists so the ABDM certification matrix is **derived from NHA's workbooks and never
transcribed** (ADR-100). It worked. It was also, quietly, short.

Its case-id filter listed eight prefixes — `CRT_`, `HIP_`, `HIU_`, `TAGGING`, `HFR-`, `HPR-`,
`LINK_`, `DISC_`, `CONS_` — and skipped every row whose id matched none of them, without a word.
What it skipped:

- `VRFY_ABHA_*` and `PROF_ABHA_*` — the **entire second half of the M1 workbook**, the half its own
  title names (_ABHA Creation **and Verification**_). Fourteen mandatory cases.
- `Health_RECORD_*` and `USER_INIT_LINK_*` in M2 — seven more.
- In M3, a second problem with the same effect: `HIU_FLOW_107`–`113` and `HIU_FLOW_301` put their id
  in **column B** rather than column E, because NHA merges the requirement cell above them. The
  parser read column E only, so eight of M3's sixteen cases did not exist as far as it was
  concerned.

M1 reported **12** mandatory cases. It has **26**. M2 reported 8; it has 15. M3 reported 8 cases in
total; it has 16. The evidence pack built on those numbers was therefore complete _about the wrong
set_, and every case it never listed was never evidenced, never tested and — as it turned out —
partly never built.

Reading the fourteen recovered M1 cases against the code found a real defect underneath, not just a
counting one. Verifying a patient by their **ABHA address** does not use `/v3/profile/login/*` at
all. NHA routes it through a separate PHR web-login family with its own search, its own verify, its
own profile path and its own card path. `abdm.constants.ts` already recorded all five paths — three
of them were referenced by nothing. So an ABHA-address verification requested its OTP correctly and
then asked `/v3/profile/account` for the profile with a token that endpoint does not accept.

It passed every test we had, because `AbdmMockProvider` returned a full profile from `loginVerify`
for both families. The mock was **kinder than the sandbox** — exactly the failure ADR-130 was
written about, reappearing one API family over.

Two more gaps came out of the same read. An ABHA number and an ABHA address must each be verifiable
by an Aadhaar OTP _and_ by the ABHA-linked mobile — `VRFY_ABHA_101`/`_102` against `_201`/`_202`,
all four mandatory — and the OTP system was hard-coded one per identifier, so half of each pair had
no way to run. And `resendOtp` handled enrolment only: on a verification transaction it either
refused or, given a mobile, called the enrolment mobile-update endpoint, which answers with a
transaction that verification cannot use. `VRFY_ABHA_305` and `_405` are mandatory, and every other
verification case repeats the same resend clause. There was also no resend **button** anywhere in
the Portal — the endpoint had never had a caller.

### Decision

**An unmatched case id is a bug in the parser, not a non-case.** `abdm-audit.ts` now claims every
prefix the five workbooks use, reads an id from column B when column E is empty and column B holds
one, inherits a merged requirement down a group (and _only_ across that layout shift, so a
deliberately blank sibling like `CRT_ABHA_115` stays `unstated` rather than inheriting an invented
requirement), and reports `"Any one of them is mandatory"` as **conditional** — one requirement over
a group of eight, not eight requirements. Anything that looks like an id and is claimed by no prefix
is printed to stderr as a warning that the matrix is short by that many.

**The API family travels with the token.** `getProfile` and `getAbhaCard` take a `family`, an
ABHA-address verification fetches its profile from the PHR path, and the ABHA-address search runs
first — before any OTP — because that is the order NHA's own workbook lists and because it is what
tells us an address is unknown before a patient is promised a message.

**The mock is no kinder than the sandbox.** `AbdmMockProvider.loginVerify` returns a token and no
profile for the PHR family, so a service that skips the follow-up call fails locally instead of at
NHA.

**The OTP system is the caller's choice where NHA offers one**, carried on the transaction as a
`_aadhaar` suffix on the flow so the verify call sends the scope pair its OTP was requested with.
The Portal exposes it as _Send the OTP to_, shown only for the two identifiers that have a choice.

**Resend is a button, for every flow that sends an OTP.** `ResendOtpButton` — sixty seconds, at most
twice, re-checked server-side on the transaction — sits beside Verify on both the enrolment and the
verification steps, and `resendOtp` repeats the original login request rather than an enrolment one.

**A malformed identifier is refused here.** A mobile that is not a valid Indian mobile, an Aadhaar
that is not twelve digits and an ABHA number that is not fourteen never reach the registry;
`VRFY_ABHA_301` and `_401` test the refusal, not the acceptance. And "no ABHA against this
identifier" is a **404 naming the case**, not the 502 it used to be — a national registry answering
correctly is not an upstream failure, and the operator needs a reason to move to creation.

### Consequences

- The certification matrix moved from 27 to 49 mandatory cases across M1–M3. Nothing regressed; the
  requirements were always there.
- Every M1, M2 and M3 case now carries an evidence entry. The 80 that remain `NOT EVIDENCED` are all
  M4, which this change does not touch.
- The three PHR paths were **confirmed against the sandbox** on 03/09/2026 with an unassigned ABHA
  address (`npm run abdm:check -- --phr`): the search read our ciphertext and judged the address,
  the profile and card paths asked for the token we withheld. The paths are right. A complete round
  trip still needs a real ABHA address and a real OTP, so those cases stay `unverified`.
- An unassigned ABHA address answers `400 ABDM-9999 Invalid ABHA Address` rather than an empty
  method list. Both shapes are handled; the observed one is recorded where the code branches.

## ADR-140 - Four callbacks nobody was serving, and a transaction id that was ours to invent

**Status:** Accepted · **Date:** 03/09/2026 · **Extends:** ADR-089 (M2 linking), ADR-091 (data flow), ADR-092 (M3 HIU), ADR-107 (Fidelius key material), ADR-139 (the recovered M1 cases)

### Context

ADR-139 fixed the tool that reads NHA's _workbooks_. This is what came out of reading NHA's
**API documents** — `M2_Document_16_02_2026` and `M3_Dcoument_16_02_2026`, both committed under
`docs/abdm/ABDM V3 Doc/` and until now never extracted. The Postman collections that
`abdm.constants.ts` was reconciled against carry only the calls a participant _makes_. The documents
carry the other half: the callbacks HIE-CM makes back.

Four were missing outright.

| Path                                        | What it answers                           | Document  |
| ------------------------------------------- | ----------------------------------------- | --------- |
...
**This class of gap cannot fail loudly.** Every call in this protocol is answered on a callback
rather than on the connection that asked, so an unserved callback is silence: nothing throws,
nothing is logged, and every local check that plays both halves of the exchange passes. `abdm:m2check`
(27 checks) and `abdm:m3check` (36 checks) were both green throughout.

Two of the four were merely unconfirmed acknowledgements. The other two were load-bearing.

**`on-status` is where a consent status actually arrives.** `pollConsentRequest` read
`consentRequest.status` out of the synchronous response to `consent/v3/request/status`. The document
has that call answer `200` carrying nothing and deliver the status to the callback. A request would
therefore have sat in "waiting for the patient…" for ever, whatever the patient did — and the poll,
the visible fallback for a callback that never came, would have been the thing that never worked.

**`on-request` is the only statement of the transaction id, and we were inventing our own.** The
request body for `health-information/request` has no transaction-id field: the consent manager
assigns one and returns it on the callback, and the HIP then pushes records under it. Our code did
`const transactionId = randomUUID()`, stored it, and looked up an inbound push by it — over a comment
reading _"the transaction id is the only handle an inbound push carries, and it is ours — we minted
it."_ Against a real HIP every delivery would have matched no row and been discarded as unrecognised,
with a warning nobody would connect to the cause. In mock mode both halves are us, so it round-tripped
perfectly. This is the ADR-130 lesson again at a different layer: **the test double agreed with us
instead of with the counterparty.**

Reading the documents also settled two older uncertainties and found a third defect:

- The six M2/M3 inbound paths that carried an "unverified against an official collection" warning for
  weeks are **confirmed correct**. The warnings are gone because a document says so, not because a
  convention held.
- `keyMaterial.dhPublicKey.keyValue` on the **HIU** side sent the raw public key. ADR-107 applied
  NHA's _"certain HIUs only accept the public key in the base64-encoded X.509 format"_ to the HIP half
  and missed this one. The constraint is symmetric — a HIP may be equally strict about what an HIU
  sends it.

### Decision

**Serve all four, guarded like every other inbound route.** Each carries `requireAbdmGateway`
(ADR-109), answers before doing the work, and drops an unattributable callback with a log line rather
than a 5xx — answering 5xx makes NHA retry something that can never succeed.

**The transaction id is ABDM's.** `requestRecords` still writes a value up front, because the row
exists before the request goes out, but it is named a **placeholder** in the code and
`recordDataRequestAck` replaces it the moment the consent manager states the real one — correlated on
our own `requestId`, the one handle both sides held. A refusal on that callback closes the transfer
instead of leaving it waiting for a delivery that will never come.

**The two acknowledgements are audit entries, not tables.** Neither changes clinical or consent
state; each says whether a message already sent was accepted. A table whose only reader is a support
question is a migration with no user. The originating `requestId` is now recorded in the audit
metadata of the _send_, so the pair reads back together — which meant giving the care-context notify
a `requestId` in its body, since it had none and nothing else identifies which notify an
acknowledgement answers.

**Read both of NHA's spellings.** The SMS acknowledgement uses `resp` and a bare `status`; the
care-context one uses `response` and `acknowledgement.status`. Both are accepted, because picking one
would silently record half the acknowledgements as anonymous failures.

**The guard is asserted as a property.** A new test walks the router's own stack and fails if _any_
mounted route lacks `requireAbdmGateway`, rather than naming today's routes — the four added here
would otherwise have been four chances to forget.

### Consequences

- M2 and M3 gain 4 endpoints: 16 inbound gateway routes, up from 12.
- `HIP_INIT_NOTIFY_HIECM` moves from "sent, outcome unknown" to sent-and-acknowledged.
- The M3 data-fetch cases (`HIU_FLOW_106`–`113`) stay `unverified`, but for a different reason than
  yesterday: the code path is now correct rather than only untested.
- 874 backend tests pass (was 859); 15 new in `abdm/__tests__/callbacks.test.ts`, including the
  before-and-after of the transaction-id defect.
