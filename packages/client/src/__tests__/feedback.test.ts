...
const { ApiRequestError, NetworkError, TimeoutError } = await import('../errors');
...
describe('describeError', () => {
  ...
  it('NEVER surfaces a server message on a 5xx', () => {
    const leaky = new ApiRequestError(
      500,
      'ERROR',
  ...
  it('rejects developer-shaped messages and falls back', () => {
    const stacky = new ApiRequestError(
      400,
      'ERROR',
      'Error: at Object.handler (/srv/app/dist/x.js:12:9)',
    );
  ...
  it('rejects a bare error code as user-facing copy', () => {
    expect(
      describeError(new ApiRequestError(409, 'CONFLICT', 'CONFLICT_STATE')).description,
    ).not.toBe('CONFLICT_STATE');
  });

  it('maps each status to its own copy and dedupe key', () => {
    expect(describeError(new ApiRequestError(401, 'UNAUTHORIZED', '')).title).toBe(
      'Session expired',
    );
    ...
    expect(describeError(new ApiRequestError(429, 'TOO_MANY_REQUESTS', '')).title).toBe(
      'Too many requests',
    );
    expect(describeError(new ApiRequestError(422, 'VALIDATION_ERROR', '')).title).toBe(
      'Check the details',
    );
  });
...
describe('successMessage', () => {
  ...
  it('builds copy from the response when given a formatter', () => {
    const msg = successMessage(
      { drugName: 'Amoxicillin', quantity: 2 },
      (r: never) => {
