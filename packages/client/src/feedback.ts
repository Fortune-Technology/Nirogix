...
function usableMessage(message: string | undefined): string | null {
  ...
  if (
    /\b(at\s+\w+\.\w+|Error:|stack|ECONNREFUSED|ETIMEDOUT|SQLSTATE|syntax error)\b/i.test(trimmed)
  )
    return null;
  return trimmed;
}
...
export function describeError(err: unknown): Described {
  ...
  if (err instanceof NetworkError) {
    return {
      ...
      description:
        'You appear to be offline, or the service is unavailable. Check your connection and try again.',
      dedupeKey: 'network',
    };
  }
