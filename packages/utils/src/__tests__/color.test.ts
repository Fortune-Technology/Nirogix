...
import { contrastRatio, ensureContrast, parseHexColor, relativeLuminance, toHexColor } from "../color";
...
describe("ensureContrast", () => {
  ...
  test("darkens a mid-tone that is only marginal", () => {
    ...
    expect(contrastRatio(parseHexColor(out)!, { r: 255, g: 255, b: 255 })).toBeGreaterThanOrEqual(5);
  });
  ...
  test("honours a custom threshold and background", () => {
    ...
    expect(contrastRatio(parseHexColor(strict)!, { r: 255, g: 255, b: 255 })).toBeGreaterThanOrEqual(15);
  });
