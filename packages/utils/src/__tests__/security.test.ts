...
describe("buildContentSecurityPolicy", () => {
  ...
  it("allows the bundler what it needs in development, and nothing extra in production", () => {
    expect(directive(buildContentSecurityPolicy({ nonce: "n", development: true }), "script-src")).toContain(
      "'unsafe-eval'",
    );
    expect(directive(buildContentSecurityPolicy({ nonce: "n" }), "script-src")).not.toContain("'unsafe-eval'");
  });

  it("adds the app's own API origin to connect-src, and websockets only in development", () => {
    const prod = buildContentSecurityPolicy({ nonce: "n", connectSrc: ["https://api.nirogix.com"] });
