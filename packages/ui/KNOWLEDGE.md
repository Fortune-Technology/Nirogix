...
- **`Menu` / `MenuItem` / `MenuCheckboxItem`** — the one dropdown primitive (Esc + outside-click close, arrow-key roving focus); `ActionMenu` (row actions, destructive items routed through a confirmation) and `ConfirmDialog` (focus-trapped, scroll-locked) build on it.
