...
const DRUGS: ComboboxOption[] = [
  { value: "d1", label: "Amoxicillin 500 mg", description: "Capsule", meta: "₹4.00", keywords: "amox antibiotic" },
...
describe("Combobox", () => {
  ...
  it("reports the option when one is picked, and null when the text is the user's own", () => {
    ...
    expect(onChange).toHaveBeenLastCalledWith("Paracetamol 650 mg", expect.objectContaining({ value: "d2" }));
  ...
  it("re-links free text that is typed out to exactly match a master row", () => {
    ...
    expect(onChange).toHaveBeenLastCalledWith("  metformin 500 MG ", expect.objectContaining({ value: "d3" }));
  });
  ...
  it("wraps from the end of the list back to the start", () => {
    ...
    fireEvent.keyDown(field(), { key: "ArrowDown" });
    fireEvent.keyDown(field(), { key: "Enter" });
    expect(onChange).toHaveBeenLastCalledWith("Amoxicillin 500 mg", expect.objectContaining({ value: "d1" }));
  });
