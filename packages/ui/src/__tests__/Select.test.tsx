...
describe('Select', () => {
  ...
  it('only offers a search box once the list is long enough to need one', () => {
    const { unmount } = render(
      <Select label="Short" value="" onChange={() => {}} options={DOCTORS} />,
    );
    open('Short');
  ...
  it('clears to empty without opening the list', () => {
    const onChange = vi.fn();
    render(
      <Select label="Department" value="p1" onChange={onChange} options={DOCTORS} clearable />,
    );
  ...
  it('survives a group name that appears in two non-adjacent runs', () => {
    ...
    render(
      <Select label="Permission" value="" onChange={() => {}} options={unsorted} searchable />,
    );
    open('Permission');
