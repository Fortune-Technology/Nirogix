...
const columns: Array<Column<AuditEntry>> = [
  ...
  { key: "action", header: "Action", accessor: (r) => r.action, cell: (r) => <span className="font-medium text-fg">{r.action}</span> },
...
function Eod() {
  ...
  const load = useCallback(async (date: string) => {
    ...
    try {
      ...
      const res = await api.listAudit({ from: date, to: date, pageSize: 100, sortBy: "createdAt", sortDir: "desc" });
      setRows(res.data);
  ...
  return (
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard label="Events" value={loading ? null : total} hint={truncated ? `latest ${rows.length} shown below` : undefined} />
        <StatCard label="Warning or critical" value={loading ? null : severe} hint="Needs a look" invertDelta />
