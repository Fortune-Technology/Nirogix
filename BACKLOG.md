...
| I-5 | **DNS, TLS and edge for `nirogix.com`** | Cloudflare zone setup + the staging/production VMs | The host map and cutover checklist are written (`resources/domains.md` §9). Outstanding: create the records, confirm Universal SSL covers each second-level host, put Cloudflare Access in front of the three staging hosts, set `CORS_ORIGINS` per environment, and publish SPF/DKIM/DMARC on `mail.nirogix.com` before the first real send (also gated by I-1). `docs`, `status` and `cdn` stay reserved and unrouted until built. |
