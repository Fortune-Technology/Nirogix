...
**The spine of the product.** Before any module is built, this document defines _who does what first,
what happens next, and what each actor sees and does at every step_ — from Platform Owner → Organization/Hospital
→ Admin → Doctors/Staff → Patients. Modules are built to serve these journeys, never in isolation.

> Read alongside `architecture.md` (the _how_), `projectrequirementdoc.md` (the _what_), and
> `development-plan.md` (the _when_). On any conflict the upstream docs win; this doc is the journey
...
- **System Super Admin** sees **aggregated data across every tenant** (platform-wide stats) — but only _aggregates_, never another tenant's row-level PHI (see §5, ADR-023).
...
### 1.1 Onboard a hospital — _"what do I do first?"_ ✅ Built (§20A / A1, A3)

1. **Register the organization/hospital** — Tenants → _Onboard tenant_: org code + name. ✅
...
_for_ the patient at the desk). The patient's own journey:
...
- **Module entitlement** = _whether_ a tenant has a capability (bought/provisioned by the System Admin). Gate: `requireModule`.
- **RBAC permission** = _whether a given user_ may do an action within an entitled module. Gate: `requirePermission`. Overridable per user (GRANT/DENY, time-bound). DENY always wins.
