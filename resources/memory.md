...
- **The AI Portal is an authorization boundary before it is a product (ADR-053).** Built 16/08/2026: `ai.portal.access` is held by **no role** and granted per person, a patient principal is refused **by type** before any permission is read, entry is audited, and `capabilities` is an empty list with a test asserting it stays empty. Nothing about AI is marketed until a capability is scoped, and anything touching diagnosis or treatment needs the CDSCO classification check recorded first.
