...
- **Theme:** `data-theme` on `<html>` — **`light` for every first visit, on every app, regardless of the OS preference** (ADR-079); `dark` exists only as an explicit user choice, persisted per app and restored on the next load. `prefers-color-scheme` is never consulted. **Both surfaces support Light + Dark.** Every colour is a token with a Light and Dark value; never define a colour only inside a `[data-theme]` block. A pre-hydration script paints the stored theme before first paint (no flash). Marketing persists under `mk-theme`, the Portal under `hms-theme`.
