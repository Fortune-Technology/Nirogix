...
- **Menu:** the one dropdown/popover primitive — surface + hairline + radius md + `shadow-md`, accent check marks, Esc/outside-click close, arrow-key roving focus. Column visibility, faceted filters and `ActionMenu` are all built on it.
- **ActionMenu:** the row-actions affordance — a `MoreHorizontal` trigger; destructive items sit in the danger tone and route through `ConfirmDialog`.
...
- **Toast / notification (the one API-feedback surface):** a single `Toaster` + `toast()` primitive in `@hms/ui`, used by **both** surfaces for every API result. Card-like: surface + hairline + radius lg + soft floating shadow, a Lucide status icon, title (≤ 6 words) + optional description carrying the **API's own message**, optional action, always a dismiss control. Variants map to the semantic tokens in §2 — `success` · `error` (danger) · `warning` · `info` · `loading` — with the tinted background, the semantic foreground for the icon, and body text in `ink`. Bottom-right on desktop, top-full-width on mobile (never covering the primary action or the `BackToTop` control); stacked with a small cap and de-duplication; entrance/exit ≤ 200ms and static under `prefers-reduced-motion`; `role="status"` (polite) for routine messages, `role="alert"` (assertive) for errors; success auto-dismisses (~5s), errors persist until dismissed; Esc dismisses the focused toast. Behaviour/ergonomics follow the shadcn/ui Toast pattern; the implementation is token-driven `@hms/ui` code, not an added package (ADR-026).
...
10. **Reuse before you build.** Check `@hms/ui` first, then the app's shared components. Tabular data always renders through the Standard DataTable, configured per module — never a `PatientTable` / `BillingTable` of its own. Row actions use `ActionMenu`, destructive actions use `ConfirmDialog`, and empty/loading/error use the shared states. A pattern appearing a second time gets extracted in the same change.
