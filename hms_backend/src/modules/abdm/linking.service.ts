 ...
 */
export function toPatientBlocks(contexts: AbdmCareContext[], patientReference: string): PatientBlock[] {
...
export async function linkPendingForPatient(
  ...
  if (!facility?.hipId) return { linked: 0, reason: 'This hospital has no HFR facility id configured' };
  ...
  try {
    ...
    for (const ctx of pending) await markLinkResult(tenantId, ctx.id, { linked: false, error: message });
 ...
 * do the right thing.
 */
export async function linkPendingCareContexts(tenantId: string, limit = 50): Promise<{ patients: number; linked: number }> {
 ...
 */
export async function notifyCareContextUpdate(tenantId: string, careContextId: string): Promise<void> {
  ...
  if (!context) throw new AppError(404, 'ABDM_CARE_CONTEXT_NOT_FOUND', 'That care context was not found');
  ...
  if (!facility?.hipId || !token) throw new AppError(422, 'ABDM_NOT_READY', 'No facility id or link token is available');

  await hipPost(
    HIP_GATEWAY_PATHS.notifyCareContext,
    {
      notification: {
        careContext: { careContextReference: context.referenceNumber, patientReference: context.abhaAddress },
  ...
  await writeAudit({
    ...
    metadata: { hiTypes: context.hiTypes },
  });
}
...
export async function notifyPatientBySms(
  ...
  if (!/^[6-9]\d{9}$/.test(phone)) return { sent: false, reason: 'No valid mobile number for this patient' };

  const facility = await getFacilityConfig(tenantId);
  if (!facility?.hipId) return { sent: false, reason: 'This hospital has no HFR facility id configured' };

  await hipPost(
    HIP_GATEWAY_PATHS.smsNotify,
    {
      notification: { hip: { id: facility.hipId, name: facility.facilityName ?? 'Hospital' }, phoneNo: phone },
...
export async function recordLinkCallback(input: {
  ...
  const facilityRows = await db.select().from(abdmFacilityConfig).where(eq(abdmFacilityConfig.hipId, input.hipId)).limit(1);
  ...
  const reverted = await runWithTenant(tenantId, (tx) =>
    tx
      ...
      .set({ status: 'pending', linkedAt: null, lastError: (input.error ?? input.status ?? 'Link refused').slice(0, 300), updatedAt: new Date() })
      .where(and(eq(abdmCareContexts.tenantId, tenantId), eq(abdmCareContexts.abhaAddress, input.abhaAddress), eq(abdmCareContexts.status, 'linked')))
  ...
  await writeAudit({
    ...
    severity: 'warning',
    metadata: { abhaAddress: input.abhaAddress, error: input.error, status: input.status, reverted: reverted.length },
  });
