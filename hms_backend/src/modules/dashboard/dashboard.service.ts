...
export async function getOrgSummary(tenantId: string): Promise<OrgSummary> {
  const { userCount, doctorCount, patientCount, apptCount, branchRows } = await runWithTenant(
    tenantId,
    async (tx) => {
      const u = (
      ...
      const p = (
        await tx.select({ c: count() }).from(providers).where(eq(providers.tenantId, tenantId))
      )[0];
      const pt = (
      ...
      return {
        ...
        doctorCount: Number(p?.c ?? 0),
        patientCount: Number(pt?.c ?? 0),
        apptCount: Number(ap?.c ?? 0),
        branchRows,
      };
    },
  );
