  ...
  request: {
    query: PaginationQuerySchema.extend({
      search: z
        ...
        .max(120)
        .optional()
        .openapi({ description: 'Free-text over action / path / resource type' }),
      severity: z.string().optional().openapi({
        description: 'Comma-separated severities (multi-select): info,notice,warning,critical',
      }),
      from: z
        .string()
        .optional()
        .openapi({ description: 'Inclusive lower bound over created_at (YYYY-MM-DD)' }),
      to: z
        .string()
        .optional()
        .openapi({ description: 'Inclusive upper bound over created_at (YYYY-MM-DD)' }),
      sortBy: z
        .enum(['createdAt', 'action', 'severity', 'statusCode'])
        .optional()
        .openapi({ description: 'Allow-listed sort column (default createdAt)' }),
      sortDir: z
        .enum(['asc', 'desc'])
        .optional()
