...
- `POST /api/v1/encounters/{id}/amend` · `POST /api/v1/encounters/{id}/amend/cancel` (`emr.encounter.amend`) — correcting a **signed** consultation (ADR-134). Amending copies the signed note into `encounter_amendments.snapshot` *before* anything becomes editable and moves the encounter to a third status, `amending`, which saves through the ordinary `PUT`; re-signing goes back through `/sign`, closes the amendment and stores the fields that actually differ. The amendment belongs to whoever opened it — not to whoever holds the key — so a second user editing through someone else’s stated reason is refused. Cancelling is allowed only while nothing has been saved. `amending` counts as **has been signed** everywhere downstream (patient history, pharmacy pending), and `encounter.signed` is published once, by the first signature.
