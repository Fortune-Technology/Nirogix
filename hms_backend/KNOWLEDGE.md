...
- **Env:** add new config to `EnvSchema` in `config/env.ts`, then to `.env.example` **and** `.env` in the same change — same keys, same order, live and uncommented, 1–2 line comment (CLAUDE.md → _Environment files_). A blank value is stripped before validation, so it means “not configured” and can never block the boot; never a real secret in `.env.example`.
...
- **A second ABDM OTP needs a reason (ADR-131):** `requiresMobileVerification` is decided from the **numbers** — `requestedMobile !== mobileOnRecord && mobileMatchesAadhaar !== true` — not from the flag alone. The gateway adapter had read the flag as `Boolean(x ?? undefined)`, which cannot yield `undefined`, so an ABDM response omitting `mobileMatchesAadhaar` (most of them) meant "does not match" and every patient got a second OTP on the phone that had just received the first. `asOptionalBoolean` keeps the flag tri-state; absent means _ask_, and matching numbers mean _do not_. The mock returns the **Aadhaar-linked** mobile rather than echoing the requested one, because echoing it made the two never differ and the decision never exercised.
- **A multi-step ABDM flow merges, it does not replace (ADR-130):** `completeWithProfile` lays each step's profile _over_ what the transaction already holds, per field, skipping null/undefined/blank, and identifiers fall back to the transaction's own — because Aadhaar returns the whole record, the mobile OTP after it returns a token, and resolving a chosen ABHA returns a token. Treating the newest response as the whole profile blanked the registration form on the last step of every two-step flow. The **candidate account list is persisted** on the transaction when the operator must choose, since it is the only description of that person the flow produces; the chosen account's `name` is used **whole** rather than split on a space. The **mock provider answers as sparsely as the sandbox** on those second calls, deliberately — a mock kinder than the real system hid this from 306 passing tests.
- **A past is seeded once, the present on every run (ADR-133).** `seedClinicalStory` writes history behind a marker; `seedTodayQueue` and `seedTodayArrivals` run on **every** seed and guard on the day itself — _does this hospital already have a visit dated today?_ — so an environment seeded last week still has a live OPD board, vitals queue and arrivals board this morning, and a deploy refreshes them without a reset. A collision degrades rather than aborts: a taken slot becomes a walk-in, a patient already in the OPD is skipped, an arrival whose booking fails is announced without one, each counted in the report. A dataset also declares `workflow` (`vitalsMode` and the hospital's own consultation/case words) — **the Vitals queue has rows in no other mode**, so a dataset that set none left the screen permanently blank.
- **The staging dataset is shaped like development (ADR-132):** `QAHOSP` (all modules, 42 days × 3 visits, 28 charts), `QACLINIC` (pharmacy and laboratory off — module entitlement and tenant isolation), `QACLOSED` (suspended), plus the `NIROGIX` vendor org. Its first two patients stay `QA Patient One`/`Two` in that order because E2E asserts on them (ADR-114), and `scripts/__tests__/stagingDataset.test.ts` checks the hand-written data for the mistakes hand-written data actually acquires — a department code a service points at that no longer exists, a reused phone number (the key a patient is identified by), an unknown specialty, a provider linked to a renamed account. **Growing the dataset does not grow an already-seeded environment's history:** the clinical story runs once per tenant, so new hospitals arrive complete but an existing one gains only the new records — a bigger history for it needs `--reset`.
- **Seeding on deploy, and never overwriting (ADR-122):** the staging deployment workflow runs `db:seed:staging` right after `db:migrate`, so staging catches up with a new table, a new reference record or a new demo record on the deploy that ships it. What makes that safe is that the engine **converges on _present_, never on the dataset's values**. Every record is found by a **stable key** — tenant code, user email, branch/department code, provider registration number, lab-test/service code, supplier/drug name, patient **phone**, request phone, notification idempotency key, override (user, permission) — never by display name; found means _left exactly as it is_, missing means created, and nothing is ever updated. Actions with no record of their own (organisation profile, branding, the three public-form toggles, the clinical story, column backfills) carry a row in **`seed_markers`** (platform-managed, no `tenant_id`, no RLS — like `tenants`), written _after_ the work succeeds so a failed deploy retries. `applyBackfills()` fills a newly added column **only where it is NULL**, once, behind its own marker. Production is kept out by four independent guards: branch + VM, `requireEnvironment('staging')`, the `DATABASE_URL` shape check, and the workflow's own `NODE_ENV=staging` assertion; CI never passes `--reset`. The report distinguishes created from kept — `created nothing; kept 113 existing` is the healthy steady state.
...
- **Table (tenant-scoped, RLS):** `tenant_capability_entitlements` (migration `0031`) — **deny-by-exception**: a capability is ON whenever its module is entitled and no _effective_ override disables it, so an empty table = every capability on, and the introducing migration needed **no backfill** (existing behaviour preserved byte-for-byte). Never physically deleted; branch-nullable.
...
- **Indian SMS is template-bound, and the code reflects that.** The header `NIROGX` is DLT-approved
  ...
  carries provider-template values (distinct from `data`, which renders the _local_ `{{template}}`), and
...
- **Treatment cases (`patient_cases`, ADR-116)** — the episode a run of visits belongs to, and what a `follow_up` arrival actually follows. `C-`-numbered on a per-tenant advisory lock, same as visit numbers. `visits.case_id` is nullable **and stays that way**: most consultations are one-offs, and forcing a case on every walk-in fills charts with one-visit episodes nobody closes. **Several open cases per patient are allowed** — a long-term condition and a fresh injury are separate — so the product surfaces what is open at the moment a new one would be created rather than refusing the second. Check-in opens a case through `openCaseTx` **inside the visit transaction**, so a check-in that fails afterwards cannot leave an orphan. `assertCaseUsableTx` is the gate on attaching a visit: the case must belong to _this_ patient and still be open. Closing is refused while a visit under it is live, needs a reason, and never deletes (invariant #6); reopening clears the reason from the row, which is why the audit entry carries it.
...
- **Patient documents (`patient_documents`, ADR-119)** — the link between a file and the patient, visit or case it concerns. `file_metadata` deliberately knows nothing about who a file is _about_, which is why this is a link table rather than columns on the file store. `file_id` is a plain uuid with **no FK** (the convention `tenant_branding.logo_file_id` already uses): files soft-delete and are retained, so a hard FK would block that or cascade the attachment away — a deleted file leaves its attachment reading `(file removed)`. **Three ids are checked server-side**: the file must be this tenant's (the store is shared infrastructure — RLS protects the row only if the service looks for it), and a named visit or case must belong to _this_ patient. No new permissions: `file.document.view` / `file.document.upload` already answer the question and the front desk holds both. Archived with a reason, never deleted; the upload itself still goes through `POST /files`.
- **Pricing dimensions (ADR-121)** — the fee schedule keys on five things: doctor (16), department (8), case type (4), consultation type (2), arrival type (1); powers of two, so the ordering is total. The two new ones carry the **hospital's own words**, held as `text[]` in `hospital_workflow_config` and resolved branch-then-organization. Every writer goes through `assertConsultationType` / `assertCaseType`, so an unconfigured value cannot reach a rule, a case or a visit — and an **empty vocabulary rejects everything**, because a hospital that has configured no types is one where a type is meaningless. `case_type` lives on the **case**, not the visit, and is deliberately not a field on the check-in body: a client-stated case type would be a client-chosen price. Removing a word from a vocabulary is refused while an active rule prices it (the error names the types); retired rules are ignored. Everything starts empty and empty means the question is not asked, so a hospital that never opens the screen is unaffected.
...
- `GET|POST|DELETE /api/v1/me/signature` (`platform.signature.manage`) — a person's own **electronic** signature (ADR-137). Versions, never edits: uploading again retires the previous row rather than repointing it, and `encounters.signature_id` / `lab_results.signature_id` pin **which version signed**, so a clinician changing theirs next year cannot change what a document printed today. There is deliberately **no `/users/{id}/signature`** — an administrator holding every permission cannot upload one in somebody else's name because the operation cannot be expressed. An uploaded image, never a cryptographic digital signature; the API says so in `kind`.
- `GET /api/v1/imports` · `GET /api/v1/imports/history` · `GET /api/v1/imports/{module}/template` · `POST /api/v1/imports/{module}/preview` · `POST /api/v1/imports/{module}/commit` — bulk import (ADR-138), authorised by **the imported module's own** permission, checked in the handler because the module is a path parameter. Master data only (medicines, tests, services, doctors, departments, patients); appointments, visits, consultations, prescriptions, results, invoices and payments are deliberately absent, because a spreadsheet that creates them skips the workflow that gates on payment and signs a note. Preview and commit both take the file — no staging area — so the preview describes the file rather than reserving anything.
- `POST /api/v1/encounters/{id}/amend` · `POST /api/v1/encounters/{id}/amend/cancel` (`emr.encounter.amend`) — correcting a **signed** consultation (ADR-134). Amending copies the signed note into `encounter_amendments.snapshot` _before_ anything becomes editable and moves the encounter to a third status, `amending`, which saves through the ordinary `PUT`; re-signing goes back through `/sign`, closes the amendment and stores the fields that actually differ. The amendment belongs to whoever opened it — not to whoever holds the key — so a second user editing through someone else’s stated reason is refused. Cancelling is allowed only while nothing has been saved. `amending` counts as **has been signed** everywhere downstream (patient history, pharmacy pending), and `encounter.signed` is published once, by the first signature.
...
- **The legacy column stays.** `visits.department` is the original free-text field; check-in writes the department's _name_ into it as well as setting `department_id`, so every existing read keeps working. Deprecated, and goes when no row needs it.
...
- **Two permissions.** Listing the queue is `patient.record.view`; approving and rejecting are `patient.record.create`. They cannot share one: approving creates a chart, and a role that may only look — a cashier — would otherwise never see that anything had arrived. (`org_admin` holds both since ADR-125 and can work the queue; the split is still the right shape, for every role that does not.) `listRequests` **projects field by field** rather than spreading the row, so `tenant_id`, `submitted_ip` and `reviewed_by` stay internal.
...
**Verification is two identifiers times two OTP systems, and one of them is a different API family
(ADR-139).** NHA makes all four mandatory: an ABHA **number** and an ABHA **address** must each be
verifiable by an Aadhaar OTP (`VRFY_ABHA_101`, `_102`) and by the ABHA-linked mobile (`_201`,
`_202`). Two consequences that are not guessable:

- The **OTP system is a choice**, carried on the transaction as a `_aadhaar` suffix on `flow`, because
  the verify call must send the scope pair its OTP was requested with (`abha-login` + `aadhaar-verify`
  or + `mobile-verify`). Hard-coding one system per identifier leaves two mandatory cases with no way
  to run. The Portal shows it as _Send the OTP to_, only for the two identifiers that have a choice.
- An ABHA **address** does not use `/v3/profile/login/*` at all. It goes through the PHR web-login
  family, which has its own `search` (asked FIRST, before any OTP), its own verify, its own
  `abha-profile` and its own `phr-card`. Its verify returns a **token and no demographics** — the
  profile is a second call to that family’s own path — and a token minted there is refused by
  `/v3/profile/account`. So `getProfile` and `getAbhaCard` take a `family`, and `AbdmMockProvider`
  returns no profile for `phr` so a service that skips the follow-up fails locally rather than at NHA.
  The three paths were confirmed against the sandbox on 03/09/2026: `npm run abdm:check -- --phr`.

**Resend is a button, and it belongs to the flow that sent the OTP.** `CRT_ABHA_106` and
`VRFY_ABHA_305`/`_405` all say _"System may activate the Resend OTP button maximum 2 times after 60
seconds"_. `resendOtp` repeats the **original** request — an enrolment resend re-runs enrolment, a
verification resend re-runs the login request — because the enrolment mobile-update endpoint answers
with a transaction a verification cannot verify against. The identifier is re-supplied by the caller,
since the transaction stores a masked hint and nothing else. `ResendOtpButton` in the Portal is the
only caller; before ADR-139 the endpoint had none.

**A malformed identifier never reaches the registry**, and "no ABHA against this identifier" is a
**404 that names the case**, not a 502 — a national registry answering correctly is not an upstream
failure, and the operator needs a reason to move to creation (`VRFY_ABHA_301`, `_302`, `_401`, `_403`).
...
**Credentials.** One client id/secret for the _application_ (server config); one HFR facility id per
_hospital_ (`abdm_facility_config`, tenant-scoped, sent as `X-HIP-ID`). The facility id is what the
...
decides how this module is read and how it fails: an endpoint we do not serve produces **silence**,
not an error. Nothing throws, nothing logs, and a self-test that plays both halves of the exchange
passes. Four callbacks were missing for weeks with 859 tests green and two milestone self-tests at
27 and 36 checks.

So: **the Postman collections are not the contract.** They hold only the calls we make. The
callbacks live in NHA’s API documents (`docs/abdm/ABDM V3 Doc/`), and those are the thing to
reconcile `abdm.constants.ts` against when anything changes.

Two of the four were load-bearing rather than cosmetic:

- **`/api/v3/hiu/consent/request/on-status`** — `consent/v3/request/status` answers `200` carrying
  nothing. The status arrives here. `pollConsentRequest` still reads the synchronous body because
  some environments fill it in, but a poller written against the response alone never sees a change,
  and the poll exists precisely as the fallback for a callback that never came.
- **`/api/v3/hiu/health-information/on-request`** — the **only** statement of the transaction id.
  The data request body has no field for one: the consent manager assigns it and the HIP pushes
  under it. `requestRecords` writes a **placeholder** because the row exists before the request goes
  out, and `recordDataRequestAck` replaces it, correlated on our own `requestId`. Inventing the
  transaction id meant every real push matched nothing — invisible in mock mode, where both halves
  are us.

The two M2 acknowledgements (`/api/v3/links/context/on-notify`, `/api/v3/patients/sms/on-notify`) are
recorded as **audit entries, not tables**: neither changes clinical or consent state, and the
originating `requestId` now sits in the audit metadata of the send so the pair reads back together.
Both of NHA’s spellings are accepted — `resp` + `status` in §4.3.9, `response` +
`acknowledgement.status` in §4.3.7 — because picking one records half the acknowledgements as
anonymous failures.

**The guard is a property, not a list.** A test walks the router’s own stack and fails if any mounted
inbound route lacks `requireAbdmGateway` (ADR-109). Naming the routes would have meant four more
chances to forget.
...
best-effort, so no ABDM failure can break a clinical action, and it only _records_ that a shareable
...
because this must happen _at all_ rather than exactly once — it also sweeps M2's consents, which had
...
## Default list ordering (ADR-136)

Ordering is chosen **per workflow**, not once: a queue, a schedule, a ledger and a directory want
different things at the top, and `ORDER BY created_at DESC` is the right answer for roughly one
of them.

| List             | Leads with                                   | Why                                                                                                         |
| ---------------- | -------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| Appointments     | Future soonest-first, then past newest-first | A schedule is read forwards. `scheduled_at DESC` put the furthest-away booking on top.                      |
| Treatment cases  | `open` before `closed`                       | Ordered on the status's **meaning** — sorting the column put `closed` first, against the comment beside it. |
| Invoices         | Money still owed                             | A billing list is worked through; a settled invoice is a receipt, found by number.                          |
| Lab worklist     | Workflow stage, then FIFO within it          | Finished reports are the bulk of the table and were sitting on the work.                                    |
| OPD queue        | Token number                                 | Unchanged — the queue _is_ the order.                                                                       |
| Pharmacy pending | FIFO                                         | Unchanged — already filtered to `ordered`; whoever waited longest is next.                                  |
| Patients         | Newest registered                            | Unchanged — a directory, not a queue.                                                                       |

**User sorting overrides the default, and a client never names a column.** `db/sort.ts` parses the
DataTable's own `key:dir,key:dir` format and resolves each key against an allowlist the module
publishes (`APPOINTMENT_SORT`, `INVOICE_SORT`), keyed by the same column keys the Portal uses. An
unknown key is dropped — a stale bookmark gives somebody the default list, not a 422 — and
dropping every key falls back to the workflow-aware default. Adding sort to another endpoint is an
allowlist plus one `orderBy` line.
