...
function AvailabilityManager() {
  ...
  async function toggle(item: api.AvailabilityItem) {
    ...
    try {
      ...
      setItems((prev) => prev.map((it) => (it.ref === item.ref ? { ...it, isAvailable: next } : it)));
    } catch {
    ...
    <div className="flex flex-col gap-4">
      <Card header="Per-hospital availability">
        <p className="mb-4 text-sm text-fg-muted">
          Choose which of your organisation&apos;s items each hospital offers. Turning an item off here removes it
      ...
      <Card header={branchName ? `${branchName}: ${typeLabel}` : "Items"}>
        {loading ? (
          ...
          <p className="text-sm text-fg-subtle">No items of this type yet. Add them in the relevant catalogue first.</p>
        ) : (
