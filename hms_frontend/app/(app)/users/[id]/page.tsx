      ...
      <TableActions label={`Actions for the ${r.name} role`}>
        <TableAction
          ...
          confirm={{
            ...
            description:
              'They lose every permission this role grants, unless another role grants it too.',
    ...
    },
    {
      ...
      cell: (o) => (
        <span className="text-fg-muted">
          {o.validUntil ? formatDate(o.validUntil) : 'Permanent'}
        </span>
      ),
    },
        ...
        actions={
          <Link href="/users">
            <Button variant="ghost">
              <ArrowLeft size={16} strokeWidth={2} /> All users
            </Button>
          </Link>
        }
      />
            ...
            <Select
              ...
              options={[
                { value: 'active', label: 'Active' },
                {
                  value: 'suspended',
                  label: 'Suspended',
                  description: 'Cannot sign in; the account is kept',
                },
              ]}
            />
          </Can>
        ...
        <Can perm={PERMISSIONS.RBAC_MANAGE}>
          {assignable.length > 0 && (
            <div className="mt-4 flex items-center gap-2">
              ...
              <Button
                size="sm"
                disabled={!assignKey || busy}
                onClick={() =>
                  run(async () => {
                    await api.assignUserRole(id, assignKey);
                    setAssignKey('');
                  })
                }
              >
        ...
        {user.wildcard ? (
          ...
          <div className="flex max-h-56 flex-wrap gap-1.5 overflow-y-auto">
            {user.permissions.map((p) => (
              <span
                key={p}
                className="rounded-token bg-surface-2 px-2 py-1 font-mono text-xs text-fg-muted"
              >
                {p}
              </span>
            ))}
            ...
            <Select
              ...
              options={[
                { value: 'DENY', label: 'DENY', description: 'Always wins over a role grant' },
                {
                  value: 'GRANT',
                  label: 'GRANT',
                  description: 'Adds a permission the roles do not give',
                },
              ]}
            />
            <Button
              size="sm"
              disabled={!ovPerm || busy}
              onClick={() =>
                run(async () => {
                  await api.addUserOverride(id, { permission: ovPerm, effect: ovEffect });
                  setOvPerm('');
                })
              }
            >
