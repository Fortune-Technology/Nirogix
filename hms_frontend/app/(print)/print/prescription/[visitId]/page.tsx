...
function PrescriptionDocument({ visitId }: { visitId: string }) {
  ...
  useEffect(() => {
    api
      ...
      .then(setEnc)
      .catch((e) =>
        setError(e instanceof api.ApiRequestError ? e.message : 'Could not load the consultation.'),
      );
  }, [visitId]);
  ...
  return (
    <>
      <PrintToolbar
        onBack={() => router.push(`/opd/${visitId}`)}
      ...
      <PrintDocument
        ...
        reference={
          <>
            {enc.signedAt ? (
              <div>Signed {formatDateTime(enc.signedAt)}</div>
            ) : (
              <div>
                <strong>Draft (not signed)</strong>
              </div>
            )}
      ...
      >
            ...
            <PrintTable
              columns={[
                { key: 'drug', header: 'Medicine', cell: (r: RxRow) => r.drugName },
                {
                  key: 'dose',
                  header: 'Dose',
                  cell: (r: RxRow) => r.dose ?? emptyLabel('unspecified'),
                },
                {
                  key: 'freq',
                  header: 'Frequency',
                  cell: (r: RxRow) => r.frequency ?? emptyLabel('unspecified'),
                },
                {
                  key: 'route',
                  header: 'Route',
                  cell: (r: RxRow) => r.route ?? emptyLabel('unspecified'),
                },
                {
                  key: 'duration',
                  header: 'Duration',
                  cell: (r: RxRow) => r.duration ?? emptyLabel('unspecified'),
                },
                {
                  key: 'instructions',
                  header: 'Instructions',
                  cell: (r: RxRow) => r.instructions ?? emptyLabel('unspecified'),
                },
              ]}
        ...
        {/* The signature the encounter PINNED at signing, not the doctor's current one
            (ADR-137) — a clinician who uploads a new signature next year does not change what
            this prescription showed today. An unsigned or unconfigured one prints a blank line. */}
        <PrintSignatures
          signatures={[
            {
              label: enc.providerName ? `${enc.providerName}: Signature` : "Doctor's signature",
              imageUrl: enc.signature?.imageUrl ?? null,
              imageAlt: enc.signature?.signedByName
                ? `Signature of ${enc.signature.signedByName}`
                : 'Signature',
              signedAt: enc.signedAt ? formatDateTime(enc.signedAt) : undefined,
            },
          ]}
        />
      </PrintDocument>
