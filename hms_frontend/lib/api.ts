...
import type { SortState } from '@hms/ui';
...
export async function updateProvider(id: string, patch: UpdateProviderRequest): Promise<Provider> {
  return request<Provider>(`/providers/${id}`, {
    method: 'PATCH',
    body: patch,
    feedback: { success: 'Doctor updated.' },
  });
}

export async function assignProviderSpecialty(
  id: string,
  body: AssignSpecialtyRequest,
): Promise<void> {
  await request(`/providers/${id}/specialties`, {
    method: 'POST',
    body,
    feedback: { success: 'Specialty assigned.' },
  });
}
...
export async function listRegistrationRequests(
  ...
  return (
    await request<{ requests: RegistrationRequestItem[] }>(
      `/registration-requests?status=${status}`,
    )
  ).requests;
}
...
export async function approveRegistrationRequest(
  ...
  return request<{ patientId: string }>(`/registration-requests/${id}/approve`, {
    ...
    body: opts,
    feedback: {
      success: opts.existingPatientId ? 'Linked to the existing patient.' : 'Patient registered.',
    },
  });
}
...
export async function updatePatient(
  id: string,
  patch: Partial<CreatePatientRequest> & { status?: string },
): Promise<Patient> {
  return request<Patient>(`/patients/${id}`, {
    method: 'PATCH',
    body: patch,
    feedback: { success: 'Patient updated.' },
  });
}
...
/**
 * The DataTable's sort state as the wire format both ends already use — `key:dir,key:dir`,
 * the same string its URL state writes (ADR-136).
 *
 * Sending it is not optional politeness: a server-mode table offers sorting on every column
 * that knows its value, so a call that drops this leaves the user with a header arrow that
 * moves and rows that do not.
 */
export function sortParam(sort: SortState[] | undefined): string | undefined {
...
export async function bookAppointment(
  ...
  return request<{ id: string; status: string }>('/appointments', {
    method: 'POST',
    body,
...
export async function updateVisitStatus(
  ...
  return request<Visit>(`/visits/${id}/status`, {
    method: 'PATCH',
    body,
    feedback: { success: 'Visit updated.' },
  });
}
...
export async function previewConsultationFee(opts: {
  ...
  return request<ResolvedConsultationFee>(`/fee-rules/preview${qs ? `?${qs}` : ''}`, {
    feedback: false,
  });
}

export async function createFeeRule(body: CreateFeeRuleRequest): Promise<ConsultationFeeRule> {
  return request<ConsultationFeeRule>('/fee-rules', {
    method: 'POST',
    body,
    feedback: { success: 'Fee rule added.' },
  });
}
...
export async function getConsentStatus(patientId: string): Promise<AbdmConsentStatus> {
  return request<AbdmConsentStatus>(`/abdm/history/${patientId}/consent-status`, {
    feedback: false,
  });
}
...
export async function listCases(
  opts: { patientId?: string; status?: 'open' | 'closed' } = {},
): Promise<PatientCase[]> {
  const q = new URLSearchParams();
  if (opts.patientId) q.set('patientId', opts.patientId);
  if (opts.status) q.set('status', opts.status);
...
export async function openCase(body: OpenCaseRequest): Promise<PatientCase> {
  return request<PatientCase>('/cases', {
    method: 'POST',
    body,
    feedback: { success: 'Case opened.' },
  });
}
...
export async function closeCase(id: string, body: CloseCaseRequest): Promise<PatientCase> {
  return request<PatientCase>(`/cases/${id}/close`, {
    method: 'POST',
    body,
    feedback: { success: 'Case closed.' },
  });
}
...
export async function recordVitals(body: RecordVitalsRequest): Promise<VitalsRecord> {
  return request<VitalsRecord>('/vitals', {
    method: 'POST',
    body,
    feedback: { success: 'Vitals recorded.' },
  });
}
...
export async function listVitalsQueue(
  opts: { branchId?: string; pending?: boolean } = {},
): Promise<VitalsQueueEntry[]> {
  const q = new URLSearchParams();
  if (opts.branchId) q.set('branchId', opts.branchId);
...
export async function recordPayment(id: string, body: RecordPaymentRequest): Promise<Invoice> {
  return request<Invoice>(`/invoices/${id}/payments`, {
    method: 'POST',
    body,
...
export async function addInvoiceLine(
  ...
  return request<Invoice>(`/invoices/${invoiceId}/lines`, {
    method: 'POST',
    body,
    feedback: { success: 'Item added to the bill.' },
  });
}
...
export async function createService(body: CreateServiceRequest): Promise<Service> {
  return request<Service>('/services', {
    method: 'POST',
    body,
    feedback: { success: 'Service added.' },
  });
}
...
export async function updateService(id: string, patch: UpdateServiceRequest): Promise<Service> {
  return request<Service>(`/services/${id}`, {
    method: 'PATCH',
    body: patch,
    feedback: { success: 'Service updated.' },
  });
}
...
export async function listReferrals(
  opts: { status?: string; toDepartmentId?: string; patientId?: string } = {},
): Promise<Referral[]> {
  const q = new URLSearchParams();
  if (opts.status) q.set('status', opts.status);
...
export async function createReferral(body: CreateReferralRequest): Promise<Referral> {
  return request<Referral>('/referrals', {
    method: 'POST',
    body,
    feedback: { success: 'Referral created.' },
  });
}

export async function cancelReferral(id: string): Promise<Referral> {
  return request<Referral>(`/referrals/${id}/cancel`, {
    method: 'POST',
    feedback: { success: 'Referral cancelled.' },
  });
}
...
export async function listProviderSchedules(providerId: string): Promise<ScheduleWindow[]> {
  return (await request<{ windows: ScheduleWindow[] }>(`/providers/${providerId}/schedules`))
    .windows;
}

export async function setProviderSchedules(
  providerId: string,
...
export async function listBookingRequests(status = 'pending'): Promise<BookingRequestItem[]> {
  return (await request<{ requests: BookingRequestItem[] }>(`/booking-requests?status=${status}`))
    .requests;
}
...
export async function rejectBookingRequest(id: string, reason?: string): Promise<void> {
  await request<void>(`/booking-requests/${id}/reject`, {
    method: 'POST',
    body: { reason },
    feedback: { success: 'Request rejected.' },
  });
...
export async function createSupplier(body: CreateSupplierRequest): Promise<Supplier> {
  return request<Supplier>('/suppliers', {
    method: 'POST',
    body,
    feedback: { success: 'Supplier added.' },
  });
}

export async function updateSupplier(
  id: string,
  patch: Partial<CreateSupplierRequest> & { isActive?: boolean },
): Promise<Supplier> {
  return request<Supplier>(`/suppliers/${id}`, {
    method: 'PATCH',
    body: patch,
    feedback: { success: 'Supplier updated.' },
  });
}

export async function adjustStock(
  drugId: string,
  body: { batchId?: string | null; delta: number; reason: string },
): Promise<Drug> {
  return request<Drug>(`/drugs/${drugId}/adjust`, {
    method: 'POST',
    body,
    feedback: { success: 'Stock corrected.' },
  });
}
...
export async function verifyLabResult(id: string): Promise<LabOrder> {
  return request<LabOrder>(`/lab-orders/${id}/verify`, {
    method: 'POST',
    feedback: { success: 'Report verified.' },
  });
}
...
export async function openEncounter(visitId: string): Promise<Encounter> {
  return request<Encounter>('/encounters/open', {
    method: 'POST',
    body: { visitId },
...
export async function saveEncounter(id: string, body: SaveEncounterRequest): Promise<Encounter> {
  return request<Encounter>(`/encounters/${id}`, {
    method: 'PUT',
    body,
    feedback: { success: 'Consultation saved.' },
  });
}

export async function signEncounter(id: string): Promise<Encounter> {
  return request<Encounter>(`/encounters/${id}/sign`, {
    method: 'POST',
    feedback: { success: 'Consultation signed.' },
  });
}
...
/**
 * Your own signature (ADR-137). There is no `getSignature(userId)` because there is no such
 * endpoint: the API acts on the authenticated user and takes no user id.
 */
export async function getMySignature(): Promise<MySignature> {
  return request<MySignature>('/me/signature', { feedback: false });
}

/**
 * Multipart, so it goes through `send` rather than `request`: the shared client JSON-stringifies
 * every body and sets `application/json`, which would destroy a `FormData`. Same shape as
 * `uploadFile` above — the boundary header is deliberately left to the browser.
 */
export async function uploadMySignature(
  file: File,
): Promise<SignatureVersion & { imageUrl: string | null }> {
  const form = new FormData();
  form.append('file', file);
  const headers: Record<string, string> = {};
  const token = client.getAccessToken();
  if (token) headers['Authorization'] = `Bearer ${token}`;
  const res = await send('/me/signature', {
    method: 'POST',
    headers,
    credentials: 'include',
    body: form,
  ...
  return (await res.json()) as SignatureVersion & { imageUrl: string | null };
}

export async function removeMySignature(): Promise<void> {
  await request<void>('/me/signature', {
    method: 'DELETE',
    feedback: { success: 'Signature removed from new documents.' },
  });
}

/**
 * Bulk import (ADR-138). Preview and commit are multipart, so they go through `send` rather
 * than `request` — the shared client JSON-stringifies every body, which would destroy a
 * `FormData`. The mapping rides beside the file as a JSON string.
 */
export async function getImportOptions(): Promise<ImportOptions> {
  return request<ImportOptions>('/imports', { feedback: false });
}

export function importTemplateUrl(moduleKey: string): string {
  return `/imports/${encodeURIComponent(moduleKey)}/template`;
}

/**
 * Downloads the sample CSV. Fetched rather than linked because the endpoint needs the bearer
 * token, which an `<a download>` cannot carry.
 */
export async function downloadImportTemplate(moduleKey: string, filename: string): Promise<void> {
  const headers: Record<string, string> = {};
  const token = client.getAccessToken();
  if (token) headers['Authorization'] = `Bearer ${token}`;
  const res = await send(importTemplateUrl(moduleKey), {
    method: 'GET',
    headers,
    credentials: 'include',
  });
...
export async function previewImport(
  ...
  return importUpload<ImportPreview>(`/imports/${encodeURIComponent(moduleKey)}/preview`, file, {
...
export async function commitImport(
  ...
  return importUpload<ImportCommitResult>(
    `/imports/${encodeURIComponent(moduleKey)}/commit`,
    file,
    {
      duplicateStrategy,
      mapping: mapping ? JSON.stringify(mapping) : undefined,
...
export async function listImportRuns(moduleKey?: string): Promise<ImportRun[]> {
  const q = moduleKey ? `?module=${encodeURIComponent(moduleKey)}` : '';
  return request<ImportRun[]>(`/imports/history${q}`, { feedback: false });
}
...
export async function receiveStock(drugId: string, body: ReceiveStockRequest): Promise<Drug> {
  return request<Drug>(`/drugs/${drugId}/stock`, {
    method: 'POST',
    body,
    feedback: { success: 'Stock received.' },
  });
}
...
export async function createLabTest(body: CreateLabTestRequest): Promise<LabTest> {
  return request<LabTest>('/lab-tests', {
    method: 'POST',
    body,
    feedback: { success: 'Lab test added.' },
  });
}
...
export async function collectLabSample(id: string): Promise<LabOrder> {
  return request<LabOrder>(`/lab-orders/${id}/collect`, {
    method: 'POST',
    feedback: { success: 'Sample marked collected.' },
  });
}

export async function enterLabResult(id: string, body: EnterResultRequest): Promise<LabOrder> {
  return request<LabOrder>(`/lab-orders/${id}/result`, {
    method: 'POST',
    body,
    feedback: { success: 'Result saved.' },
  });
}
...
export async function assignUserRole(id: string, roleKey: string): Promise<void> {
  await request(`/users/${id}/roles`, {
    method: 'POST',
    body: { roleKey },
    feedback: { success: 'Role assigned.' },
  });
}

export async function removeUserRole(id: string, roleKey: string): Promise<void> {
  await request(`/users/${id}/roles/${roleKey}`, {
    method: 'DELETE',
    feedback: { success: 'Role removed.' },
  });
}
...
export async function addUserOverride(
  ...
  await request(`/users/${id}/overrides`, {
    method: 'POST',
    body,
    feedback: { success: 'Override added.' },
  });
}

export async function revokeUserOverride(id: string, overrideId: string): Promise<void> {
  await request(`/users/${id}/overrides/${overrideId}`, {
    method: 'DELETE',
    feedback: { success: 'Override revoked.' },
  });
}
...
export async function updateBranch(
  ...
  return request<Branch>(`/branches/${id}`, {
    method: 'PATCH',
    body: patch,
    feedback: { success: 'Branch updated.' },
  });
}
...
export async function listDepartments(
  ...
  if (opts.branchId) q.set('branchId', opts.branchId);
  const qs = q.toString();
  return (await request<{ departments: Department[] }>(`/departments${qs ? `?${qs}` : ''}`))
    .departments;
}
...
export async function resetBranding(): Promise<Branding> {
  return request<Branding>('/branding', {
    method: 'DELETE',
    feedback: { success: 'Branding reset to the default.' },
  });
}
...
export async function uploadBrandingAsset(kind: 'logo' | 'favicon', file: File): Promise<Branding> {
  ...
  const token = client.getAccessToken();
  if (token) headers['Authorization'] = `Bearer ${token}`;
  const res = await send(`/branding/${kind}`, {
    method: 'POST',
    headers,
    credentials: 'include',
    body: form,
...
export async function verifyAbhaAadhaarOtp(body: {
  ...
  return request<AbhaVerificationResult>('/abdm/enrolment/aadhaar/verify', {
    method: 'POST',
    body,
  });
}
...
export async function requestAbhaMobileOtp(body: {
  transactionId: string;
  mobile: string;
}): Promise<AbdmOtpSent> {
...
export async function verifyAbhaMobileOtp(body: {
  transactionId: string;
  otp: string;
}): Promise<AbhaVerificationResult> {
...
export async function startAbhaVerification(body: {
  ...
  consentGiven: true;
  branchId?: string;
  /** Omitted, the OTP goes to the ABHA-linked mobile — which is what every caller had before. */
...
export async function selectAbhaAccount(body: {
  ...
  return request<AbhaVerificationResult>('/abdm/verification/select-account', {
    method: 'POST',
    body,
  });
}
...
export async function dismissAbdmVerification(transactionId: string): Promise<void> {
  ...
  await request<void>(`/abdm/transactions/${transactionId}/dismiss`, {
    method: 'POST',
    feedback: false,
  });
}
...
export async function linkAbhaToPatient(body: {
  transactionId: string;
  patientId: string;
}): Promise<Patient> {
...
export async function requestAbdmHistory(body: {
  patientId: string;
  providerId: string;
}): Promise<AbdmHistoryRequest> {
  ...
  return request<AbdmHistoryRequest>('/abdm/history/request', {
    method: 'POST',
    body,
    feedback: false,
  });
...
export async function listAbdmHistoryRequests(patientId: string): Promise<AbdmHistoryRequest[]> {
  ...
  const data = await request<{ requests: AbdmHistoryRequest[] }>(`/abdm/history/${patientId}`, {
    feedback: false,
  });
...
export async function refreshAbdmHistoryRequest(requestId: string): Promise<AbdmHistoryRequest> {
  return request<AbdmHistoryRequest>(`/abdm/history/${requestId}/refresh`, {
    method: 'POST',
    feedback: false,
  });
}
...
export async function fetchAbdmExternalRecords(patientId: string): Promise<{ requested: number }> {
  ...
  return request<{ requested: number }>(`/abdm/history/${patientId}/fetch`, {
    method: 'POST',
    feedback: false,
  });
}
...
export interface AbdmHprEnrolment {
  ...
  hprId: string | null;
  status:
    'not_started' | 'aadhaar_verified' | 'mobile_verified' | 'registered' | 'already_registered';
  statusMessage: string | null;
...
export async function listAbdmFacilityRegistrations(): Promise<AbdmFacilityRegistration[]> {
  const data = await request<{ registrations: AbdmFacilityRegistration[] }>(
...
export async function submitAbdmFacilityRegistration(
  branchId?: string | null,
): Promise<AbdmFacilityRegistration> {
...
export async function searchAbdmFacilities(
  ...
  return request<AbdmFacilitySearchResult>(`/abdm/registry/facility/search?${q}`, {
    feedback: false,
  });
}
...
export async function startAbdmHprEnrolment(body: {
  ...
  return request<AbdmHprEnrolment>('/abdm/registry/professional/start', {
    method: 'POST',
    body,
    feedback: false,
  });
...
export async function verifyAbdmHprAadhaarOtp(body: {
  providerId: string;
  otp: string;
}): Promise<AbdmHprEnrolment> {
...
export async function sendAbdmHprMobileOtp(body: {
  providerId: string;
  mobile: string;
}): Promise<void> {
  await request<void>('/abdm/registry/professional/mobile-otp/send', {
    method: 'POST',
    body,
    feedback: false,
  });
...
export async function verifyAbdmHprMobileOtp(body: {
  providerId: string;
  otp: string;
}): Promise<AbdmHprEnrolment> {
...
export async function listAbdmHprEnrolments(): Promise<AbdmHprEnrolment[]> {
  const data = await request<{ enrolments: AbdmHprEnrolment[] }>('/abdm/registry/professionals', {
    feedback: false,
  });
...
export async function recordAbdmFacilityVerification(body: {
  ...
  return request<AbdmFacilityRegistration>('/abdm/registry/facility/verification', {
    method: 'POST',
    body,
  });
}

export async function exportAbdmBulk(
  kind: 'professionals' | 'facilities',
): Promise<AbdmBulkExport> {
...
export async function importAbdmBulk(
  ...
  return request<AbdmImportOutcome>(`/abdm/registry/bulk/${kind}`, {
    method: 'POST',
    body: { rows },
    feedback: false,
  });
...
export async function listAbdmConsents(): Promise<{
  consents: AbdmHeldConsent[];
  history: AbdmConsentEvent[];
}> {
  return request<{ consents: AbdmHeldConsent[]; history: AbdmConsentEvent[] }>('/abdm/consents', {
    feedback: false,
  });
}
...
export async function lookupAbdmAbha(identifier: string): Promise<{
  ...
  return request(`/abdm/history/lookup/abha?identifier=${encodeURIComponent(identifier)}`, {
    feedback: false,
  });
}

/** Resends the verification code, throttled server-side (CRT_ABHA_106, VRFY_ABHA_305/_405). */
export async function resendAbdmOtp(body: {
  transactionId: string;
  aadhaar?: string;
  mobile?: string;
  /** The identifier a VERIFICATION resend goes to; only a masked hint is stored server-side. */
  identifier?: string;
