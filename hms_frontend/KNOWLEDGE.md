...
- **Tenant branding:** the accent is a single token (`--hms-brand`). `setBrand(hex)` overrides it at runtime (inline on `<html>`), re-skinning every button/badge/link/highlight instantly in both themes — the Settings page demonstrates this.
