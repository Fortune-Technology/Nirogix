...
- **Re-running without `--reset` is safe, and "safe" means specific things.** The seeder converges
  on _present_, never on the dataset's values:
...
- **Actions with no record of their own are marked done.** Applying the organisation profile, the
  ...
  write a row in `seed_markers` and are then never repeated. The marker is written _after_ the work
...
- **`--reset` empties every tenant-scoped table**, plus the tenant list and the patient identities
  that sit outside tenancy, then reseeds. The table list is _discovered_ (every table with a
...
| **Real notification deliveries**                                                                   | The log is seeded; nothing is _sent_. A seeder that called MSG91 would put real messages on real phones.                                                                                                                                        |
